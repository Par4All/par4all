...
/*       (see file : LICENSING).                                       */
/***********************************************************************/
/** Homogenization by Bavo Nootaert, 2004-2005 **/
...
int main() {
    ...
    P2 = homogenize(P1);
    C2 = homogenize(C1);
    P = Constraints2Polyhedron(P2,WS);
    C = Constraints2Polyhedron(C2,WS);
    Matrix_Free(C1);
    Matrix_Free(P1);
    Matrix_Free(C2);
    Matrix_Free(P2);
    ...
    param_name = Read_ParamNames(stdin,C->Dimension - 1);

   
    /* Add the homogeneous parameter '_H' */
    param_name2 = (char**)malloc(sizeof(char*) * (C->Dimension));
    for (i = 0; i < C->Dimension - 1; i++){
    	param_name2[i] = (char*)malloc(sizeof(char)
				       * (strlen(param_name[i]) + 1));
	strcpy(param_name2[i], param_name[i]);
    }
    param_name2[C->Dimension-1] = (char*)malloc(sizeof(char)*3);
    strcpy(param_name2[C->Dimension-1], "_H");
    for (i = 0 ; i < C->Dimension - 1; i++ )
        free(param_name[i]);
    free(param_name);
