...
int n;
{
        ...
        g=vector(1,n);
        h=vector(1,n);
        if (r[n] == 0.0) {free_vector(h,1,n);free_vector(g,1,n);return;}
        x[1]=y[1]/r[n];
        if (n == 1) {free_vector(h,1,n);free_vector(g,1,n);return;}
        g[1]=r[n-1]/r[n];
        h[1]=r[n+1]/r[n];
        for (m=1;m<=n;m++) {
                m1=m+1;
                ...
                if (m1 == n) {free_vector(h,1,n);free_vector(g,1,n);return;}
                sgn = -r[n-m1];
                shn = -r[n+m1];
                sgd = -r[n];
                for (j=1;j<=m;j++) {
