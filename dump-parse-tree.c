...
#include "dump2PIPS.c"
//hack : in dump2PIPS we undef the "free" macro for the normal use of PIPS's functions. We have to now redefine it for normal use of gfc
...
show_symbol (gfc_symbol *sym)
{
  ...
  show_attr (&sym->attr);
  //fprintf(dumpfile,"\ndummy_order %d ",sym->dummy_order);
  //fprintf(dumpfile,"\nentry_id %d ",sym->entry_id);
  //fprintf(dumpfile,"\nrefs %d ",sym->refs);
  //fprintf(dumpfile,"\nforall_index %d ",sym->forall_index);
...
show_namespace (gfc_namespace *ns){
	//on va devoir quasiment tout réécrire dans le système de dump pour passer de la ri de gfc à celle de PIPS
	gfc_interface *intr;
	gfc_namespace *save;
	gfc_intrinsic_op op;
	gfc_equiv *eq;
	...
	/**
	 * Le problème est le suivant : on a tout, comment utiliser le moins possible de fonctions de pips afin de pouvoir construire toute la RI de PIPS
	 * dans pips on commence par un programme, une sous-routine, une fonction ou un bloc d'instructions
	 * TK_PROGRAM, TK_SUBROUTINE, TK_FUNCTION, TK_BLOCKDATA
	 *
	 * dans gfc on a des status complètement différents
	 * ns->default_type->type
	 * BT_UNKNOWN = 1, BT_INTEGER, BT_REAL, BT_COMPLEX, BT_LOGICAL, BT_CHARACTER, BT_DERIVED, BT_PROCEDURE, BT_HOLLERITH, BT_VOID
	 **/
	...
	if (ns != NULL){
		...
		//montre le nom de la procédure/programme
		if (ns->proc_name != NULL){
			show_indent ();
		}


		//on va afficher toutes les déclarations de "common"
		gfc_current_ns = ns;
		gfc_traverse_symtree (ns->common_root, show_common);

		//on affiche toutes les déclarations de variables, fonctions, paramètres, modules
	...
	//pour chaque "sous-module" on recommence en marquant bien qu'il appartient au module courant avec un CONTAINS
