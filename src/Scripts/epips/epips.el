...
; Initialize the automaton that analyses the pips output:
  ...
  (let*
	...
	(start-process "xtree"
		       ...
		       "-bg" "LightSteelBlue1" "-fg" "purple4" "-separator" "    " "-oformat" "resource"
					...
					; a temporary buffer:
       (epips-xtree-input-buffer (generate-new-buffer "epips-xtree-input"))
       )
    (save-excursion
...
(defun epips-sequential-view-command (epips-command-name
  ...
  (save-excursion
    ...
    (pop-to-buffer (aref epips-buffers epips-current-window-number))
...
(defun epips-view-command (epips-command-name epips-command-content)
  ...
  (if (equal epips-command-name epips-call-graph-view-command-name)
      (epips-ICFG-view-command epips-command-name epips-command-content)
