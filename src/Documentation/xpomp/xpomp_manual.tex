...
\newcommand{\xPOMP}{\htmladdnormallink{\textbf{xPOMP}}
  {http://www.cri.ensmp.fr/pips/xpomp_manual.html}\xspace}
...
First, as \PIPS is an interprocedural compiler, fake functions sources
(\verb|xpomp_fake.f| file)
that simply mimic their effects on arguments are provided. Secondly,
so as \HPFC to figure out that these functions are I/O functions that must
be called only by the \emph{host}, headers are provided
(\verb|xpomp_graphic_F.h| file) that must be included. Finally,
the generated code must is linked automatically to the \xPOMP library
(thanks to not yet developped hacks:-).
