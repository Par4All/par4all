...
%% On failure, the value $-1$ is returned.$RCSfile: xpomp_manual.tex,v $ (version $Revision$)
%% $Date: 1996/09/06 11:21:10 $, 

\documentclass[a4paper]{article}
\usepackage{psfig,xspace,html,verbatim}
\begin{document}
...
\begin{quotation}
  ...
  \emph{\small%
    ...
    plusieurs fenêtres X11 pour afficher des tableaux bidimensionnels
    sous forme d'images dont la couleur des pixels dépend de la valeur
    des éléments de tableau. L'interface de programmation est
    très simple et est disponible pour les langages C, Fortran et HPF.%
    }
\end{quotation}
...
\verb|''| in Fortran and not a NULL pointer), no title is displayed.
If a title is given, it is displayed below the frame with color
\verb|title_color| in a box with color \verb|background_color|. If
\verb|background_color| is $-1$, the title box background remains
transparent.
...
\end{latexonly}
\begin{rawhtml}
<a href=fractal.f>Fractal HPF program</a>
\end{rawhtml}
