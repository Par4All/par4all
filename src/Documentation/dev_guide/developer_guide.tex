...
\item Extract your own PIPS with the setup script mentionned above.
  ...
  \item[prod:] compiled production version of \texttt{pips},
    ...
    library;
  \item[pips\_dev:] Calvin's development area which points to
...
\end{verbatim}
  ...
  same name. This directory must be committed, or its creation may be
...
\item Develop, test, commit within your branch\ldots
  ...
  \item[branch-diff] what you have develop in your branch, i.e. the
    differences between your branch and the production version.
  \item[branch-info] information about your branch.
  \end{description}
\item When you're done with your developments, they are committed into
...
sh> svn commit /tmp/svn_branch.XXXXXX/
\end{verbatim}
  You must have write access to the \texttt{trunk} in order to do that.
\end{enumerate}
...
It is possible to use the \texttt{svn\_branch.sh} script
directly to create a branch for any part of pips, including the
full software.
...
status of the branch: what is its source, what was merged from
the source of joined back to it, and so on.
...
\item In the directory {\tt \$PIPS\_DEVEDIR/Scripts/dev}:
  
  \begin{itemize}
    
  \item Update the file {\tt define\_libraries.sh}\index{define\_libraries.sh}. It contains
    the shell environment variables for the PIPS libraries. To update it, just add the
    character string {\tt -lmylib} in the variable {\tt \$PIPS\_LIBS} and
    {\tt mylib} to the variable \verb|$PIPS_ORDERED_LIBS|. %$    
    Notice that the order in which the libraries are added is important :
    the new library must be added after the libraries which use its
    modules, but after the libraries whose modules it uses.
    
  \item Run the {\tt make} command to automaticall build the files
    \verb|define_libraries.make|. The command {\tt make install} then
...
\item In the directory {\tt \$PIPS\_DEVEDIR/Scripts/env}, if you need to
  introduce new environment variables or to modify old ones:
  
  \begin{itemize}
    
  \item Update the file {\tt pipsrc.ref}\index{pipsrc.ref}. It contains
    the shell environement variables for PIPS..
...
\item In the directory {\tt\$PIPS\_DEVEDIR/Documentation/pipsmake-rc/}:
