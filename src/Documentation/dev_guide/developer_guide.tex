...
Different phases are validated in distinct sub-directories.  Running the
validation uses the \texttt{pips\_validation} script mentioned above.
...
way is to provide a source file \texttt{MyTest.f}, a corresponding
\texttt{MyTest.tpips} script, and store the standard output of the
\texttt{.tpips} script into \texttt{MyTest.result/out}.
