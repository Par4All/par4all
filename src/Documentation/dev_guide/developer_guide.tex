...
It is best \textbf{not} the following validation scripts directly, 
but rather to rely on the \texttt{Makefile} in the validation directory, 
with very convenient targets including \texttt{validate accept clean}. 
Using this approach does not require the validation directory to be under 
\texttt{\$PIPS\_ROOT}.
...
\item[pips\_validate]: \index{Validate} Non-regression tests;
  ...
  \texttt{\$PIPS\_ROOT/Validation} or source files within these directories.
  See \texttt{pips\_validate -h} for help and a list of options.
  ...
  %%
  If you really need to rely on the directory containing the scripts, 
  use the \texttt{\$\{VDIR:-.\}} environment variable with a default to
  the current directory (well, I'm not that sure\ldots).

  The scripts looks for  validation scripts in the following order:
  \begin{enumerate}
  \item a corresponding. \texttt{.test} file, and executes it.

  \item a corresponding \texttt{.tpips} file which is executed as a 
    tpips script.
  ...
  \item a \texttt{default\_tpips} file and runs it with tpips. 
  ...
  is encouraged. It is very important not to validate \texttt{stderr} output 
  and pips with some level of debugging on, because these outputs are not 
  very stable from one architecture to another.

\item[manual\_accept] \index{Validate} interactive script to accept
  new validation results. Then commit these changes to the validation
  repository, if and only if the corresponding pips sources are already 
  committed!
\end{description}
