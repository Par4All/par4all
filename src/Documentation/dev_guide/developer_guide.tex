...
THIS IS NOW PARTLY OBSOLETE!

As the CRI subversion server now runs with subversion 1.5,
which includes a convenient branch management feature,
branches can be managed directly with svn, without relying on the
custom script described hereafter. This section should be updated shortly.
...
\end{verbatim}
  ...
  \verb/SVN_USERNAME/ is not necessary if the svn and unix account share
  the same login name. In this case the name in the \verb/USER/
  environment variable is used instead. This directory \textbf{must} then
  be committed.
...
\item Develop, test, commit within your branch\ldots
  ...
  \item[branch-avail] list of commits on the trunk that where not pulled
    into the branch.
  \end{description}
  For other targets about compilation, see \S~\ref{sec:local-targets}.
