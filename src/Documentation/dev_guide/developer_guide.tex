...
\emph{repository, checkout, status, commit} are necessary to develop in PIPS. 
See for instance the SVN book at \LINK{http://svnbook.red-bean.com/}
...
passes all non-regression tests that run nightly at CRI. Thus day-to-day
...
\end{verbatim}
  ...
  \item[valid] a working copy of pips non regression tests for validation.
    ...
    and the really read the \texttt{README} file to run it.
...
\end{verbatim}
  ...
  or its creation may be committed automatically with the 
  \texttt{force-create-branch} target.
...
\item[pips\_validate]: \index{Validate} Non-regression tests;
  ...
  \texttt{\$PIPS\_ROOT/../../valid} or source files within these directories.
