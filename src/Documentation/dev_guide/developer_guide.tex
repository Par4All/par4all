...
\newcommand{\PipsOldWWW}{\LINK{http://www.cri.ensmp.fr/pips/}}
\newcommand{\PipsNewWWW}{\LINK{https://freia.enstb.org/pips}}
\newcommand{\PipsDevGuidePDF}{\LINK{http://www.cri.ensmp.fr/pips/pips/developer_guide.htdoc/developer_guide.pdf}}
...
The legacy WWW site is at \PipsOldWWW and the new one is at \PipsNewWWW.
...
\subsection{Using tags to index source files and ease access in your text editor}
\label{sec:using-tags}

PIPS makefiles can generate tags index files to find the definition of
symbols in the source files with your favorite editor.

PIPS can produce tags files for VI by invoking \verb|make CTAGS| or tags
for Emacs by invoking \verb|make CTAGS|

This should be done for example in your \texttt{newgen/trunk},
\texttt{pips/trunk} and \texttt{linear/trunk}.
...
\verb/$/\emph{*}\verb|ROOT/makes| directory, \emph{except} for
...
This directory is used to build the static PIPS home page.
...
A Makefile must be created in this directory. It can be copied from an
...
the {\tt make} syntax can also be added, but now the preferred method is
to put them in \texttt{local.mk} as explained later. For local
\textsc{lex} and \textsc{yacc} files, use the \verb|$(PARSE)| and
\verb|$(SCAN)| macros, and do not forget to change the 'yy' prefixes
...
The {\tt Makefile} contains a default \texttt{recompile} entry to install
...
macros, type definitions,\dots) written by the programmer and the external
functions declared in the local source files, which are automatically
extracted using {\tt cproto}.
...
If a main program to test or use the library exists, then it necessarily
is a pass (see Section~\ref{sec:pass_organization}), or a
\lstinline|main()|. This rule is generally not respected, and this induces
cycles in case of a global recompilation. (Is this up-to-date?)
...
\verb+ri+\index{ri}. It is described in the \LaTeX\ file  {\tt ri.tex} in
...
First, create a new directory \texttt{mylib} in \texttt{src/Libs} with the
files \texttt{mylib.c} (where the code for the C \texttt{boolean
  mylib(string name)}, which corresponds to the phase to add, generally
is). Add the new directory name to the \texttt{local.mk}, so that the
recursive make will consider it.
...
\subsubsection{In directory \texttt{makes}}
...
(not likely):
  ...
  \item run the \texttt{make} command to automatically build the files and
    to install them in \verb|$PIPS_ROOT/share|.  Note that the
    \texttt{pipsrc.*sh} files created by the setup command is not
    affected, you must copy them again by hand since they were copied from
    an older version at download time. So this change will only affects
...
\item NewGen globally handles all the data structures defined in the
  ...
  generated. It is thus necessary to recompile everything in the {\bf \tt
    ...
    \$PIPS\_ROOT/Src/Lib/newgen} and, install {\tt
    \$PIPS\_DEVEDIR/Lib/pipsdbm} if it has been modified in a previous
  step. To recompile PIPS, see section~\ref{sec:building-pips}.
\end{itemize}
