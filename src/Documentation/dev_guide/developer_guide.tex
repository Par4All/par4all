...
In order to help prevent this situation, pips is checked and possibly
fully recompiled every 20 minutes.
...
At HPC Project we use GIT with central servers, so the previous points
from CRI are rather out-of-focus...
...
\item branches and tags in more than one directory (we use
  subdirectories in \texttt{branches}) are not managed~;
...
If you want to track all the branches of PIPS (and if you have the right to),
you may try:
{\scriptsize
\begin{verbatim}
git svn clone --branches=trunk/ancourt \
              --branches=trunk/bouchebaba \
              --branches=trunk/carothers \
              --branches=trunk/coelho \
              --branches=trunk/creusillet \
              --branches=trunk/duong \
              --branches=trunk/gall \
              --branches=trunk/girodias \
              --branches=trunk/godet \
              --branches=trunk/guelton \
              --branches=trunk/ief \
              --branches=trunk/irigoin \
              --branches=trunk/kalra \
              --branches=trunk/keryell \
              --branches=trunk/lplassart \
              --branches=trunk/mensi \
              --branches=trunk/muller \
              --branches=trunk/villalon \
              --branches=trunk \
              --stdlayout \
              http://svn.cri.ensmp.fr/svn/pips 
\end{verbatim}
}
to track branches in the user-specific directory. But since there also
some ``normal'' branches with no hierarchy (such as \texttt{daverio},
\verb|teraops_20090319|), there is also the \verb|--branches=trunk| for
them.

Since in the PIPS repositories there is also some quite old history (based
on RCS, because SCCS history has been lost...), the history does not fit
into classical SVN \texttt{trunk}, \texttt{branches} and \texttt{tags} and
thus cannot either be understood well by \texttt{git-svn}. So for ultimate
PIPS historian, you can have also a copy of the repository with
\texttt{git-svn} without using the
\texttt{trunk}/\texttt{branches}/\texttt{tags} convention. For this, just
use a plain \texttt{git-svn} with:
\begin{verbatim}
...
As an output the make validate creates a RESULTS directory. This directory contains:
a \texttt{SUMMARY} file, and 3 different file types (\texttt{.err}, \texttt{.diff} and
\texttt{.out}). Those files are named using the following rule : FolderName\_TestCaseName.xxx

\begin{itemize}
\item Obviusly the \texttt{SUMMARY} file contains a summary of the previous validation.
...
\subsection{Debugging PIPS C source code dynamiccaly}
