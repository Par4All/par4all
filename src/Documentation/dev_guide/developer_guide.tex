...
maintained in the \verb+Scripts/env+ directory of the sources.
...
assumed. A PIPS
architecture is to be understand not as strictly as a computer
...
This section describes the directory hierarchy of PIPS\@.  Pips resides in
...
codes are developped. Once correct, they are installed in the main
...
should offer a working pips \emph{running} environment.
...
interactive interface based on the readline library) and \texttt+wpips+
...
analyses), developper\_guide (this document!), ri (the description of the
pips Intermediate Representation).

\begin{description}
\item[Doc/manl]: local (obsolete?) man pages. 
  \verb+$PIPS_ROOT/Doc+ can be added to user MANPATH variables. %%$
  
\item[Doc/Html]: Html documentation of PIPS. Some real html files, and
  some generated from \LaTeX{} files.
\end{description}
    
\subsubsection{Runtime}

Environments needed for executing pips-compiled files.  Namely
\texttt{hpfc} and \texttt{wp65} use a PVM-based runtime.  This directory
...
\item[Src/Lib]: This directory contains the source of the several
...
\item[Src/Runtime]: This directory contains the sources of the
...
Other architecture independent tools and files used for \emph{developping}
...
\item[pips-makemake]: \index{pips-makemake}Automatic derivation of
  ...
  environment for PIPS developpers all over the PIPS, Newgen and
  Linear, Thus developping in these environments is an easier task.
...
\item[Validate]: \index{Validate} Non-regression tests; see ?!?;
  ...
  \verb+Pips/Tests/Validation+ (\verb|$PIPS_VALIDDIR|). %%$
...
Thus new versions can be developped without interfering with other
...
\verb+$PIPS_ROOT/Src+ subtree and becomes the reference. %%$

When developping or debugging several related libraries at the same
...
directory is further divided into similarily to Pips and Newgen.
Thus there are Production (\verb+$LINEAR_ROOT+) and Development directories. 
