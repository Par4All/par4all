...
\item Develop, test, commit within your branch\ldots
  ...
  pointed to by this variable. Also check for your \verb.PATH. to
...
\item When you are done with your developments, they are committed into
  ...
  {\small
\begin{verbatim}
sh> cd prod/pips  # cd in your pips trunk copy
sh> svn mergeinfo \
...
\item[pips\_force\_accept]
  The script accepts two types of argument:
  \begin{enumerate}
  \item A list of .result directories as an argument. For each of
    ...
    first.
  \item A validation \texttt{SUMMARY} in wihch all changed entry will be selected.
    As described previously, for all those entries the previous validation will be made the
    reference for future validation tests. Note that you need to run the validation
    first.
  \end{enumerate}
