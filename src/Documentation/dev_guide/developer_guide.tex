...
\item[validate-test] overwrite \texttt{test} file, so that
\texttt{svn diff} show the changes.
...
\item[CHECK] on every commit (down to the few minute scale\ldots)
  ...
  \texttt{nlpmake} makefiles on Debian Lenny (compiegne).
...
\item[AUTOC] on every hour,
  ...
  including python and fortran95, on Debian Lenny (compiegne).

  This compilation and validation requires about 20 minutes. %% (12+9).

\item[CROSS] 32-bit cross-compilation, on Debian Lenny (compiegne).

\item[NIGHT] compilation and validation on Ubuntu Karmic (canapville).

\item[PRIVE] compilation and private validation on Debian Lenny (compiegne).
...
\textbf{Note that these scripts are basically OBSOLETE.
They are superseded by local \texttt{Makefile} targets which do the same
but also parallelize the process per directory or per file.
See Section~\ref{sec:par-validate}}.
...
\verb/default_test/ or \verb/default_pyps/,
but the same rules should be applied.
Of course, it is less flexible to show information about a specific function
among hundreds.

%% FC->RK: I have not found any example of that in the validation
Another way suggested by Erka for sharing test scripts between cases
is to create a link to a script which uses its zeros argument for the
test case name.
...
Some provision are made to skip some validations under some circumstances.
\begin{itemize}
\item if PyPS is not available, define environment variable
  \verb/PIPS_VALIDATION_NO_PYPS=1/
\item if Fortran~95 support is not available, define environment variable
  \verb/PIPS_VALIDATION_SKIP=F95_syntax/ to skip this validation directory.
  Other directories can be skipped by adding their names to the definition.
\end{itemize}
...
\item[keptout] the case could not run (say Pyps or Fortran~95 support
  are not available).
\item[slow] test case skipped because tagged as slow and such cases
