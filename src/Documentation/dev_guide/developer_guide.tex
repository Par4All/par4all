...
They are accessible from anywhere in the world by the http protocol
at:
\par \centerline{\LINK{http://svn.cri.ensmp.fr/pips.html}}

There are 4 repositories for the various softwares:
\begin{description}
...
The subversion repositories are organised in standard subdirectories 
as advised for best subversion practices:
\begin{description}
...
\item[tags] tagged revisions. Note these tags are not related with the
  tags described in section~\cite{subsec:tags}.
\end{description}
...
\item on some Unises, the \texttt{/tmp} temporary directories may be cleaned
  on reboots, which can be triggered by power failures.
\end{enumerate}


\section{Developing PIPS under SVN}
\label{sec:dev-svn}

A basic understanding of svn operations and concepts such as 
\emph{repository, checkout, status, commit} are necessary to develop
in PIPS. 
...
pass all non-regression tests that run nightly at CRI. Thus day-to-day
...
\subsection{Pips development branches}
...
\item Obtain an svn user account on \texttt{svn.cri.ensmp.fr}. 
  For instance, a login \texttt{calvin} and some password may be given.
  Calvin will have his own development area under \texttt{branches/calvin}
  in the subversion repository.
\item Extract your own PIPS with the setup script mentionned above.
  The resulting directory may contains at the end of the building process:
  \begin{description}
...
\end{verbatim}
    ...
    in which case the local directory name for the checkout must be called
  ...
  \end{description}
\item Create your development branch refering to a given library directory
...
\end{verbatim}
  It creates a new branch, not yet committed, in some directory.
  \texttt{USERNAME} is not necessary if the svn and unix account share the
  same name. This directory \textbf{must} be committed, or its creation may be
  committed automatically with the \texttt{force-create-branch} target.
...
create a branch for any part of pips, including the full software.

It is possible to use \texttt{svn} directly to create and
manage your branches with copies. The \texttt{svn\_branch.sh}
adds convenient administrative data in the directory to know about the
status of the branch: what is its source, what was merged (pulled) from
the source or joined back (pushed) to it, and so on.
...
This section describes PIPS directory hierarchy. 
Here at CRI, École des mines de Paris, Pips resides in \texttt{/projects/Pips}.
This directory is owned by the \texttt{pips} user.
From this directory, several sub directories can be found. 
The most important is the \texttt{Production} directory which SHOULD contain 
an up to date copy of Pips, including \emph{all} sources, compiled libraries 
and executables, documentation, include files, and so on. 

The \texttt{bin share runtime doc} directories offer a working pips 
\emph{running} environment.
The organisation of the Newgen (\texttt{/projects/Newgen}) and C3/Linear 
(\texttt{/projects/C3/Linear}) is very similar.

\subsection{Production (\texttt{PIPS\_ROOT})}

This directory must contain the current production version,
namely pips subversion trunk directory. 
...
It also includes other architecture-dependent executables. 
Actual binaries for different architectures are stored in
...
They include shell (sh and csh), sed, awk, perl and lisp scripts! Examples:
\begin{itemize}
...
\begin{description}      
\item[pips\_validate]: \index{Validate} Non-regression tests; see ?!?;
  
  The arguments are names of subdirectories in
  \texttt{\$PIPS\_ROOT/Validation}. 
  See \texttt{Validate -h} for some help and a list of options.
  
  Tests are organized on a library basis in \texttt{Validation}
  while bugs are reported in the same way in \texttt{Bugs}.  Bugs
  ...
  If none is found, it looks for a \texttt{default\_test} file as a template
  for a \texttt{.test} file, performs some substitutions
  (\texttt{tested\_file} and \texttt{TESTED\_FILE} are substituted by the file
  name prefix in lower or upper case) and executes the generated
  test file. Otherwise it looks for a \texttt{default\_tpips} file and runs
  ...
  For maintenance purposes, the \texttt{default\_file} approach is the
...
{\huge IT MAY BE SOMEWHAT OBSOLETE DOWN FROM HERE}
...
\begin{description}
\item[build:] ???
\item[compile:] generate a running (hope so...) version;
\item[clean:] clean up the project directories;
\item[local-clean:] remove eventually some stuff in the current directory;
\item[tags:] build a TAGS index file of identifiers found in the
  subdirectories (see~\ref{subsec:tags});
\item[unbuild:] ???
\end{description}
...
Local targets are actions with a more local meaning, such as:
\begin{description}
\item[local-clean:] remove eventually some stuff in the current directory;
\end{description}
...
\subsection{Tags used for indexing the code}
\label{subsec:tags}
...
The TAGS file at the PIPS top directory is made with a \texttt{make tags}
and needs to be remade regularly to have an up to date version of the
index. The CTAGS file at the same place and built by the same target 
should be used for \textbf{vi} users.

So it is highly recommended to use for example the Emacs editor to benefit
from the tags.  You can add the PIPS TAGS file by customizing the
\texttt{Tags Table List} variable from the \texttt{Options} menu or
directly in your \texttt{\~/.emacs} with something like:
\begin{verbatim}
(setq tags-table-list 
  '("." "~/projets/PIPS/MYPIPS/prod/pips" "/usr/src/linux-2.6.16")
 )
\end{verbatim}
Afterward, you can find, by typing a \texttt{M-.} on an identifier, the
location where it is defined. Note that completion with \texttt{TAB} works
with tag searching.

TODO : They are made from the development source files of linear, NewGen
and PIPS. Indeed, since PIPS is under svn, it is not true anymore and you
need to make separate TAGS files for Linear and NewGen and put them in the
\texttt{tags-table-list} Emacs variable.
...
results are stored in \texttt{/tmp/libs.?} where the question mark stands for
several values:
\begin{itemize}
...
\texttt{myphase} with a single argument, which is the name of the 
Fortran module to analyze. It returns a boolean value, which indicates 
whether the computation has performed well. 
It therefore resembles the following dummy function:
\begin{verbatim}
...
\item Create a new directory \texttt{mylib} in \texttt{src/Libs} with
  the files \texttt{mylib.c} (where the code for the C 
  \texttt{boolean mylib(string name)}, which corresponds to the phase 
  to add, generally is). Add the new directory name to the \texttt{local.mk},
  so that the recursive make will consider it.

\item In the directory \texttt{src/Libs/mylib}:
  
  \begin{itemize}
    
  \item Create a \texttt{Makefile} (see examples in the other libraries). 
    This file must list the sources of the library and the targets to
    be build, then it must include the common makefile \texttt{main.mk}.
    
  \item Then run the commands \texttt{make} and {\tt make install}, to
    install the library.  The file \texttt{mylib.h} is installed in the
    \texttt{include} directory, %% $
    and \texttt{libmylib.a} in the \texttt{lib/\$PIPS\_ARCH} directory. 
    The library functions are now accessible by the other libraries and passes.
    
    Note that the \texttt{cproto} command is used to generate the library
    header file \texttt{mylib.h}. To force this file to be build again, 
    do a \texttt{make header}. 
    
  \end{itemize}
  
\item In directory \texttt{makes}:
  
  \begin{itemize}
    
  \item Update file \texttt{define\_libraries.mk}\index{define\_libraries.mk}. 
    It contains the ordered list of PIPS libraries for linking.
    The order in which the libraries are added is important:
    the new library must be added after the libraries which use its
    modules, but after the libraries whose modules it uses.
    
  \end{itemize}
  
\item In directory \texttt{src/Scripts/env}, if you need to
  introduce new environment variables or to modify old ones
  (not likely):
  ...
  \item possibly update \texttt{pipsrc.sh}\index{pipsrc.sh} file. 
    It contains the shell environement variables for PIPS.
    
  \item run the \texttt{make} command to automatically build the files,
    then \texttt{make install} to install?
    Note that the \texttt{pipsrc.*sh} files created by the setup 
    command is not affected, you must create it again by hand, this
    change will only affects future installations?

  \end{itemize}
  
\item In directory \texttt{src/Documentation/pipsmake-rc/}:
  ...
  \item declare the new phase to \texttt{pipsmake} by adding the necessary
    ...
    For instance, for a phase \emph{myphase} which reads a resource \emph{res1}
    andproduces a resource \emph{res2} for the current module, the rule and
    its alias are:
...
\end{verbatim}
    ...
    Note that libraries generally use the program entities, that is to say
    all the objects from the current Fortran program and which have a name.
    This resource is named \texttt{PROGRAM.entities}.
    
  \item New \emph{properties} may be necessary for the new phase. 
    They must be delared in the \texttt{properties-rc.tex} file.
    
  \item Run the command \texttt{make recompile} to derive the various files 
    \texttt{pipsmake.rc}, \texttt{phases.h}, \texttt{resources.h} and 
    \texttt{builder-map.h}, \texttt{make install} to copy them to 
    \texttt{include} or \texttt{etc}
    
  \end{itemize}
  
\item In directory \texttt{src/Libs/pipsmake}:

Execute the command \texttt{make recompile} to recompile the pipsmake library.
  
\item In directory \texttt{src/Passes}:
...
directory \texttt{src/Scripts/drivers}.
...
\item In the directory \texttt{\$PIPS\_DEVEDIR/Documentation}:
...
\end{verbatim}
  ...
  \item Run the command \texttt{make all} to now derive from 
    \texttt{pipsmake-rc.tex} the files \texttt{phases.h}, \texttt{resources.h} 
    and \texttt{builder-map.h} and to copy them into 
    \texttt{\$PIPS\_INCLUDEDIR}; to also
    derive \texttt{pipsmake.rc}, \texttt{properties.rc} and \texttt{wpips.rc} 
    and to copy them into \texttt{\$PIPS\_LIBDIR}.

  \item Run the command \texttt{make install}
...
\item In the directory \texttt{\$PIPS\_DEVEDIR/Libs/pipsdbm}: 
  ...
  \item Update the file \texttt{methods.h} by adding a line for
...
\paragraph{Remark:} It is necessary to recompile PIPS before any test,
  because changing the header files generated from \texttt{pipsmake-rc.tex}
  can lead to inconsistencies which only appear at run time
...
\LaTeX{} format. 
DDL\footnote{Newgen types are called \emph{domains\/} and are defined using a
  high level specification language called DDL for \emph{Domain Definition
...
\item The new data structure is defined in a {\em new\/} file {\tt
    mysd.ftex} or \texttt{mysd.tex}:
  
  This file has to be declared in the local {\tt Makefile} (in {\tt
    \$PIPS\_DEVEDIR/Documentation/newgen}) for its further installation in the {\bf Production} hierarchy. It merely consists in adding its name {\tt
    mysd.ftex} to the {\tt SOURCE\_FILES} list.

If the data structure is to be managed by pipsdbm, \emph{i.e.} it
is declared as a resource in pipsmake, then
\begin{verbatim}
$PIPS_DEVEDIR/Lib/pipsdbm/methods.h
\end{verbatim} %% $
must be updated.
...
\item The new data structure is added to an already existing file {\tt 
    mysd.ftex}: modifying this file is sufficient, and the above steps are
    not necessary. 
\end{enumerate}
...
\item In the directory {\tt \$PIPS\_DEVEDIR/Documentation/newgen}, 
  execute the command {\tt
    make}.  This command builds the DDL files \texttt{mysd.newgen
    mysd.spec mysd.h mysd.c} and some common files such as \texttt{specs.h}.
  If everything is ok (check that the header and source code
  for mysd look good), then do a \texttt{make install}.
 which copies the generated files to the \texttt{\$PIPS\_ROOT/Include} 
 directory;
...
\item NewGen globally handles all the data structures defined in the
  ...
  Production} directory. Do not forget to first recompile {\tt
  \$PIPS\_ROOT/Src/Lib/newgen} and,
  install {\tt \$PIPS\_DEVEDIR/Lib/pipsdbm} if
  it has been modified in a previous step. The command {\tt make-pips}
  achieves the global recompilation of PIPS. It is in the directory {\tt
    \$PIPS\_ROOT/Share} (Cf Page~\pageref{make-pips}). 
\end{itemize}
