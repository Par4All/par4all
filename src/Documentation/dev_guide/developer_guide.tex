...
\texttt{make -j1} into the \verb|$PIPS_ROOT| directory. If something
...
\item PIPS build is much more efficiently if it is stored on local disks rather
  than on remote directories accessed through the network (for instance via NFS).
...
\end{verbatim}
  ...
  \item[validation] a working copy of pips non regression tests for validation.
...
\item merge into your branch the last developments from the \texttt{trunk},
  recompile (so it will modify the executables and libraries in the
  production version) and validate;
...
Electric fence (\texttt{libefence}) also proves to be usefull when you need to stop exactly when
an illegal memory operation happens. Note that because of \lstinline|regcomp|,
I had to define the env variable \lstinline|EF_ALLOW_MALLOC_0| when debugging
tpips.
...
its behavior, the \texttt{properties} mechanism must be used. The new
...
If any, use \texttt{make} to push them into PIPS.
...
to rebuild PIPS from directory \texttt{PIPS\_ROOT} using \texttt{make
-j1 build}.
