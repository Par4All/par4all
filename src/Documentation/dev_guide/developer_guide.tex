...
\par\centerline{\LINK{http://svn.cri.ensmp.fr/svn/validation}}. 
Different phases are validated in distinct sub-directories.
Running the validation uses the \texttt{pips\_validation} script
mentionned above.
...
See various examples around to add new validation files. 
The prefered way is to provide a source file \texttt{MyTest.f},
a corresponding \texttt{MyTest.tpips} script, and store
the standard output of the tpips script into \texttt{MyTest.result/out}.

\textbf{CAUTION:} use a relative and local path in the tpips file to 
reference the source file.

\newpage
\noindent
{\huge OBSOLETE DOWN FROM HERE\ldots}
