...
Note that the parser does not have enoug information to check the validity
of the transformation, but a warning is issued if legality is doubtful. A
better method would be to represent them somehow a local functions in the
internal representation, but the implications for \texttt{pipsmake} and
other issues are clearly not all foreseen\ldots (Fabien Coelho).
