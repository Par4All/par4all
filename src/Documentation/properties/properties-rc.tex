...
%\usepackage{alltt,html,fancybox,amstext,xspace,epsfig}
\usepackage{makeidx}
\usepackage{epsfig,amstext,alltt,fullpage}
\usepackage[ps2pdf,backref,pagebackref]{hyperref}
\usepackage{xspace}
\usepackage{url}
\usepackage[hyper,procnames]{listings}
\lstset{extendedchars=true, language=Fortran, basicstyle=\footnotesize\ttfamily, numbers=left,
...
\author{Lei Zhou \hspace{2cm} 
        Fran\c{c}ois Irigoin\thanks{E-mail: {\tt irigoin@cri.ensmp.fr}}\\
        with many other contributors\\[1cm]
...
\newcommand{\PIPS}{\LINK{{\em PIPS}}{http://www.cri.ensmp.fr/pips}}
...
must specify the loop label. This default feature can be turned off and all loops
with constant loop bounds and constant increment are fully unrolled.
...
\subsection{Expression substituion}

This property is used to set the one-liner module used during expression substitution.
It must be the name of a module already loaded in pips and containing only one return instruction (the instruction to be matched)
\begin{verbatim}
...
Note that the statement number is equal to the line number in the original 
function. Note also that some phases in {\texttt{pips}
may add new statement that are not present in the original file. In this 
case the number of the statement that requires such a transformation,
is used for the added statement.
...
July 12, 1989, 
\LINK{Tech. Report A/191}{newgen-paper/newgen-paper.html}

\bibitem{JT90} P. Jouvelot, R. Triolet, 
{\em NewGen User Manual}. December
1990, 
\LINK{Tech. Report A/???.}{newgen-manual/newgen-manual.html}

\bibitem{Z94} Lei Zhou,
{\em Analyse statique et dynamique de la complÅÈxitÅÈ  des programmes
scientifiques}, 
ThÅËse de doctorat de l'universitÅÈ Pierre et Marie Curie, 14
