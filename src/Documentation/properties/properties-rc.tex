...
Fortran~90 standard. It is possible to ignore them, up to a warning, or to
substitute them by a semantically equivalent code based on return code
values. To produce an executable code, the declarations must be
regenerated: see property \verb/PRETTYPRINT_ALL_DECLARATIONS/ in
Section~\ref{subsubsection-declarations}. By default, they are ignored.
The code can still be executed if alternate returns are used only for
errors and if no errors occur.

\begin{verbatim}
PARSER_SUBSTITUTE_ALTERNATE_RETURNS FALSE
