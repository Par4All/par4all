...
compute the number of bound checks added.
...
This is a property to tell the array resizing top-down approach to use
information from MAIN program or not. If the current module is never
called by the main program, we do no array resizing for the unnormalized
declarations in this module if the property is on. However, we can 
do nothing with modules that have no callers at all, because this is a
top-down approach ! 

\begin{verbatim}
ARRAY_RESIZING_USING_MAIN_PROGRAM FALSE
\end{verbatim}
