...
ALIASING_ACROSS_TYPES FALSE
\end{PipsProp}

The property \PipsPropRef{ALIASING_ACROSS_FORMAL_PARAMETERS} is used
to handle the aliasing between formel parameters and global variables
of pointer type. When it is set to TRUE two formels parameters or a
formal one and a global pointer or two global pointers can be
aliased. If it is turned to FALSE, such pointers are assumed to be
unaliased for intraprocedural analysis and generally for root
module(i.e. modules without callers).
...
FALSE, it is assumed that two different not included
 memory access paths cannot point to the same memory locations. The safe value is TRUE.
...
Alternatetively, this matrix can be provided through the \PipsPropRef{LOOP_TILING_MATRIX} property.
The format of the matrix is a00 a01 a02,a10 a11 a12,a20 a21 a22
\begin{PipsProp}{LOOP_TILING_MATRIX}
LOOP_TILING_MATRIX ""
\end{PipsProp}

Likewise, one ca use the \PipsPropRef{LOOP_LABEL} property to specify the targeted loop.
...
The property \PipsPropRef{ARRAY_TO_POINTER_CONVERT_PARAMETERS} can be used to change the signature of the module, either not changing it,
or using pointers \lstinline|POINTER| or \lstinline|1D|.
...
ARRAY_TO_POINTER_CONVERT_PARAMETERS ""
\end{PipsProp}
