...
to true, the syntax of the file is checked by compiling it with a
Fortran 77 compiler (namely, the \verb|PIPS_FLINT| environement
variable is used for that purpose, and defaults to \verb|f77 -c -ansi|).
...
\verb|true| or \verb|yes|, the syntax of the file is checked by
compiling it with a C compiler. The environment variable \verb|PIPS_CC|
is used for that purpose. If it is undefined, the compiler chose is
\verb|gcc -c |).
...
CHECK_C_SYNTAX_BEFORE_RUNNING_PIPS FALSE
\end{PipsProp}
...
used to select the names of the new scalar variables.
