...
This pass is now obsolete. Use \PipsPassRef{restucture_control} instead.
...
Note that if the left hand side expression implies side effects, the
transformed code is not equivalent since they will be evaluated
twice. The left hand side is not checked for side effects. The
legality of the transformation is not guaranteed.
...
according to the prototype of the called functions.
\end{PipsPass}
