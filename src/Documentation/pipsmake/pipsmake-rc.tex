...
So the default option is to generate C99 code.
...
The C preprocessor is applied before the splitting.


\section{Fortran Syntactic Preprocessing}
...
\begin{PipsProp}{SIMD_AUTO_UNROLL_MINIMIZE_UNROLL} 
...
\begin{PipsProp}{SIMD_AUTO_UNROLL_SIMPLE_CALCULATION} 
...
\PIPS default behavior in various places is to evaluate symbolic constants.
While meaningful, this approach is not source-to-source compilant, so one can set property
\PipsPropRef{EVAL_SYMBOLIC_CONSTANT} to FALSE to prevent some of those evaluations.
...
iWhen set to true, only subexpressions without "+constant" terms are eligible.
...
This transformation uses the \verb/C89_CODE_GENERATION/ proprety to
