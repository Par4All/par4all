...
anymore when coping with regions.\marginpar{RK->BS: could you precise?}
%%!UserManualDocumentation
...
e.g. stack allocated or dynamically allocated, but this memory
...
\subsection{Index Set Splitting (a.k.a. Loop Peeling for the time being}
\label{subsection-loop-peeling}
\index{Loop Peeling}
\index{Index Set Splitting}
%%@UserManualDocumentation: loop_peeling
Index Set Splitting (Loop peeling) splits the loop referenced by
property \lstinline|LOOP_PEELING_LOOP_LABEL| into two loops.  The
first loop ends at an iteration designated by property
\lstinline|LOOP_PEELING_BOUND| and the second start therafter.  It
currently only works for do loops.  This transformation is always
legal. It is currently called loop peeling by mistake. But loop
splitting in combination with loop unrolling could be used to perform
loop peeling.
%%!UserManualDocumentation
\begin{PipsMake}
alias loop_peeling 'Loop peeling'
loop_peeling      > MODULE.code
        > PROGRAM.entities
        < PROGRAM.entities
        < MODULE.code
\end{PipsMake}
...
Loop peeling \emph{requires} the two following globals to be set :
\begin{itemize}
    \item \lstinline|LOOP_PEELING_LOOP_LABEL| is the loop label
\begin{PipsProp}
LOOP_PEELING_LOOP_LABEL ""
\end{PipsProp}
    \item \lstinline|LOOP_PEELING_BOUND| is the splitting bound
\begin{PipsProp}
LOOP_PEELING_BOUND ""
\end{PipsProp}
\end{itemize}

Additionnaly, \lstinline|LOOP_PEELING_PEEL_BEFORE_BOUND| can be used to accurately tell to peel the loop before or after the bound given in \lstinline|LOOP_PEELING_BOUND|
\begin{PipsProp}
LOOP_PEELING_PEEL_BEFORE_BOUND FALSE
\end{PipsProp}
...
call by the function body.  Current implementation does not work if
...
It should be followed by a call to \verb+UNSPAGHETTIFY+ and
\verb+SUPPRESS_DEAD_CODE+.

Property \verb/INLINING_CALLERS/ can be set to define the list of
...
Use this property to filter the call sites: only given module names will be considered
\begin{PipsProp}
INLINING_CALLERS ""
\end{PipsProp}
...
Unfolding is a complementary transformation of inlining~\ref{subsection-inlining}.
While inlining inlines all calls to a module in other modules, unfolding inline all calls in a module.
Thus unfolding the content of the module.
...
Outlining is the oppositie transformation of inlining~\ref{subsection-inlining}.
