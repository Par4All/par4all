...
\newenvironment{PipsProp}[1]{%
\makeatletter%
\ifx \@#1 \message{no label for this property} \else \label{prop:#1} \fi%
\makeatother%
...
are presented in Section~\ref{section-output-files}.
Section~\ref{section-feautrier-methods} is dedicated to libraries added by CEA to
...
structures from/to PIPS database. The computation of total time used to complete a
...
The following property drives how null (
\begin{PipsProp}
...
\end{PipsProp}
are displayed by Pips when generating a prettyprint.
...
USER_LOG_P TRUE
\end{PipsProp}
controls the printing of a log of the session.

\begin{PipsProp}
ABORT_ON_USER_ERROR FALSE
\end{PipsProp}
controls what to do on user errors: go ahead  or core dump (debug).
...
\end{PipsProp}
controls whether
\TPIPS{} should behave as an extended shell.
...
\PipsPropRef{PARSER_RETURN_CODE_VARIABLE} is implicitly integer because its first
...
See \PipsPropRef{PARSER_FORMAL_LABEL_SUBSTITUTE_PREFIX} which is used to generate
new variable names.
...
Function \PipsPassRef{simdizer_auto_unroll} tries to unroll the code for
...
%depending on the property variable \PipsPropRef{PRETTYPRINT_FORTRAN90} (out of
...
\marginpar{RK: which one? FI: user\_request? Not found
in validation suite.}
