...
\ref{subsubsection-out-regions}) are used instead of READ and WRITE
regions, only at call sites. Otherwise, usual regions are used. 

Arrays and scalars which are {\em logically} local to callees, but
exposed to callers because they are statically allocated or formal
parameters, are ignored. The resulting parallel loops require some
reallocation work in the callees. Ideally, the logically local
variables should be made {\em physically} local automatically,
e.g. stack allocated or dynsmically allocated, but this memory
allocation transformation is not (yet) implemented in
\PIPS{}. \marginpar{RK: Not clear (FI: To be deleted?)}

Thus \verb+in_out_regions_chains+ are not safe: the resulting code is
not equivalent to the input code. It is only useful for manual
experimentss.
...
\marginpar{RK: Sounds great ! :-) FI: Validation? Experimental?}
...
nobody should have to require intermediate targets such as ???. Ya!\marginpar{RK: hmmm; FC?}
