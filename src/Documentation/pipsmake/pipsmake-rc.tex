...
\marginpar{Comments from Nga Nguyen: The explanation of this property is
  not clear}
...
Same as inlining but always simulate the by copy passing
%%!UserManualDocumentation
\begin{PipsMake}
...
While inlining inlines all calls to a module in other modules, unfolding inline all calls in a module, thus unfolding the content of the module.
%%!UserManualDocumentation
\begin{PipsMake}
...
same as unfolding but always simulate the by copy passing
%%!UserManualDocumentation
\begin{PipsMake}
...
\subsection {Open MP Pragma}
\label{subsection-open_mp_pragma}
%%@UserManualDocumentation: OMP Pragma
This transformation will transform a parallel code into a sequential code(cf
Section~\ref{sec:cons-parall-code} for details) while generating Open MP
pragma as an extension to statements.
%%!UserManualDocumentation
...
The pragma can be of different types as defined in the ri. The following
property cam be set to str or expr. Obviously setting str pragma will be
generated as strings and setting expr pragma will be generated as expressions
\begin{PipsProp}
PRAGMA_TYPE    "str"
\end{PipsProp}
