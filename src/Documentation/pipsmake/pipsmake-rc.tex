...
\def\PipsProperty{% To avoid escaping the "_" in the property
                           % names...
...
A property, discussed later, is declared similarly in this file:
\begin{alltt}
...
\PIPSMAKE request is broken down into global times, a set of phase times
which is the accumulation of the times spent in each phase, and a set of
IO times, also accumulated through phases.
...
\index{SYSTEM\_NULL}
\index{SYSTEM\_UNDEFINED}
\index{SYSTEM\_NOT\_FEASIBLE}

This library is external and offers an independent debugging system.

The following property drives how null, undefined or non feasible
systems are displayed by Pips when generating a prettyprint.
...
\item[File Preprocessing:] if the file suffix is \verb|.F| or ,\verb|.c| then
  the file is preprocessed. By default pips uses \verb|gfortran -E| for fortran
  files and \verb|cpp -C| for C files. These preprocessors can be changed by
  setting the following environment variables:
  \begin{itemize}
  \item \verb|PIPS_FPP| for \verb|.F| files,
  \item \verb|PIPS_CPP| for \verb|.c| files.
  \end{itemize}
  Moreover the preprocessing options used can be extended with the following
  environment variables:
  \begin{itemize}
  \item \verb|PIPS_FPP_FLAGS| variable, default to \verb|-P -D__PIPS__ -D__HPFC__|,
  \item \verb|PIPS_CPP_FLAGS| variable, default to \verb|-D__PIPS__ -D__HPFC__ -U__GNUC__|.
  \end{itemize}
...
instance \texttt{gcc -E}\footnote{It can be redefined using
  ...
  variables} and broken into a new \index{compilation unit} which contains
...
\index{FOR\_TO\_WHILE\_LOOP\_IN\_CONTROLIZER}
\index{FOR\_TO\_DO\_LOOP\_IN\_CONTROLIZER}
...
To prepend a comment to the first statement of a module.
...
formal parameter with no actual counterpart\marginpar{This is
implemented for calls by reference as in Fortran. The call-by-value
implementation is on-going. See Amira Mensi.}.
...
When set to {\tt TRUE}, \texttt{EFFECTS\_POINTER\_MODIFICATION\_CHECKING} enables pointer modification checking during the computation of cumulated effects and/or RW regions. Since this is still at experimentation level, it's default value is FALSE. This property should disappear when pointer modification analyses are more mature.
...
subscript expressions only (unless regions were used to compute
...
the discrete derivatives~\cite{Irigoin2006}. The iterative fix point
...
sufficient.\marginpar{RK: Still true? FI: To be deleted?} Properties
...
\end{PipsProp}
The motivation is when applying \emph{à la} Rice parallelizing to improve
...
%%@UserManualDocumentation: internalize_parallel_code
...
%%@UserManualDocumentation: flag_kernel
...
%%@UserManualDocumentation: kernel_load_store
...
%%@UserManualDocumentation: kernelize
...
kernelize				> MODULE.code
		...
		< PROGRAM.kernels
\end{PipsMake}
The property \verb+KERNELIZE_NBNODES+ is used to set the number of nodes for this kernel.
\verb+KERNELIZE_KERNEL_NAME+ is used to set the name of generated kernel.
\verb+KERNELIZE_HOST_CALL_NAME+ is used to set the name of generated call to kernel (host side).
...
%%%@UserManualDocumentation: terapixify
...
%%@UserManualDocumentation: normalize_microcode
...
it!\marginpar{RK: Who is working on ? :-/ FI! Could be redesigned by IEF.}
...
Use the \verb+SIMD_ATOMIZER_ATOMIZE_REFERENCE+ property to make the SIMD Atomizer go wild: unlike other atomizer, it will break
 the content of a reference.
 \verb+SIMD_ATOMIZER_ATOMIZE_LHS+ can be used to tell the atomizer to atomize both lhs and rhs.
\begin{PipsProp}
SIMD_ATOMIZER_ATOMIZE_REFERENCE FALSE
SIMD_ATOMIZER_ATOMIZE_LHS FALSE
\end{PipsProp}

The \verb+SIMD_OVERRIDE_CONSTANT_TYPE_INFERENCE+ property is used by the sac library to know if it must override C constant type inference.
...
if \verb|SIMD_OVERRIDE_CONSTANT_TYPE_INFERENCE| is set to \texttt{TRUE}, the result of \lstinline|2+b| will be a \lstinline|char|.
...
%%@UserManualDocumentation: simd_remove_reductions
...
%%@UserManualDocumentation: deatomizer
...
%%!UserManualDocumentation
...
%%@UserManualDocumentation: if_conversion init
...
%%@UserManualDocumentation: if_conversion
...
%%@UserManualDocumentation: if_conversion_compact
...
%%@UserManualDocumentation: single_assignment
...
%%@UserManualDocumentation: simd_loop_const_elim
...
%%@UserManualDocumentation: simd_memory_packing
...
\marginpar{RK: IT SudParis : insert your documentation here}
...
%%@UserManualDocumentation:  phrase_distributor_init
...
%%@UserManualDocumentation:  phrase_distributor
...
%%@UserManualDocumentation:  phrase_distributor_control_code
...
%%@UserManualDocumentation:  saferscale_distributor_init
...
%%@UserManualDocumentation:  safescale_distributor
...
\subsubsection{Code distribution on GPU}
\label{sec:code-distr-gpu}

%%@UserManualDocumentation: gpu_ify
...
\end{PipsProp}
the phase generate a wrapper function to get the iteration coordinate from
...
The intrinsics function to get an i-th coordinate in the iteration space
is defined by this GNU \emph{à la} \texttt{printf} format:
\begin{PipsProp}
GPU_COORDINATE_INTRINSICS_FORMAT "P4A_VP_%d"
\end{PipsProp}
where \verb|%d| is used to get the dimension number.

Please, do not use this feature for buffer-overflow attack...


The next phase annotates loop nests with comments and guards for further generation of CUDA calls.
...
LOOP_LABEL ""
\end{PipsProp}
%%@UserManualDocumentation: flag_loops
...
%%@UserManualDocumentation: index_set_splitting
...
\lstinline|INDEX_SET_SPLITTING_BOUND| and the second start thereafter.  It
...
\begin{itemize}
\item \verb+LOOP_LABEL+ selects the loop to strip mine
\item \verb+STRIP_MINE_KIND+ can be set to 0 (fixed-size chunks) or 1 (fixed number of chunks). Negative value is used for interactive prompt.
\item \verb+STRIP_MINE_FACTOR+ controls the size of the chunk or the number of chunk depending on \verb+STRIP_MINE_KIND+.  Negative value is used for interactive prompt.
\end{itemize}
\begin{PipsProp}
STRIP_MINE_KIND -1
STRIP_MINE_FACTOR -1
...
\begin{PipsProp}
LOOP_NORMALIZE_1_INCREMENT FALSE
\end{PipsProp}
...
Generate code from a FREIA application targeting a particular
hardware accelerator. I could add resources for the module dags?
...
algebraic optimizations with on commutators.
...
If you don't want this side effect, you can check \ref{subsection-expression-optimizations} which does a pretty nice job too.
%%!UserManualDocumentation
...
It is implemented by calling \texttt{unspaghettify}
(\S~\ref{sec:unspaghettify}) with the properties
\PipsProperty{UNSPAGHETTIFY_TEST_RESTRUCTURING} and
\PipsProperty{UNSPAGHETTIFY_RECURSIVE_DECOMPOSITION} set to \texttt{TRUE}.
...
%%@UserManualDocumentation: recover_for_loop
...
for-loops into Fortran-like do-loops. Don't worry about the C-code output:
...
\verb|for_loop_to_do_loop| transformation first
(\S\ref{sec:loop-do-loop}), and only after, apply this
\verb|for_loop_to_while_loop| transformation that will transform the
...
%%@UserManualDocumentation: spaghettify
...
%%@UserManualDocumentation: full_spaghettify
...
%%@UserManualDocumentation: fsm_generation
...
%%@UserManualDocumentation: full_fsm_generation
...
%%@UserManualDocumentation: fsm_split_state
...
%%@UserManualDocumentation: fsm_merge_states
This phase is not yet implemented and do nothing right now...
...
%%@UserManualDocumentation: expression_substitution
...
through \verb|EXPRESSION_SUBSTITUTION_PATTERN| property and all
expression similar to those contained in
\verb|EXPRESSION_SUBSTITUTION_PATTERN| will be substituted to a call
...
%%@UserManualDocumentation: array_to_pointer
This transformation replaces all arrays in the module by equivalent pointer operations
For example \lstinline|float a [2][6]; a[1][3]=1.f;| becomes \lstinline|float a [2][6]; *(a+(6*1+3)*sizeof(float)|.
It is used as a preliminary phase to generate terapix assembly.
%%!UserManualDocumentation

\begin{PipsMake}
alias array_to_pointer 'Array to pointer'

array_to_pointer > MODULE.code
		> PROGRAM.entities
		< PROGRAM.entities
		< MODULE.code
\end{PipsMake}

The property \verb+ARRAY_TO_POINTER_CONVERT_PARAMETERS+ can be used to change the signature of the module.
Module similar to \lstinline|int foo(int a[20][2]);| will be turned into \lstinline|int foo(int *a);|
 The property \verb+ARRAY_TO_POINTER_FLATTEN_ONLY+ can be set to true to reduce (<>flatten) the number of dimensions of the arrays to 1, while keeping use of the \verb+[]+ operator.

\begin{PipsProp}
ARRAY_TO_POINTER_CONVERT_PARAMETERS FALSE
ARRAY_TO_POINTER_FLATTEN_ONLY FALSE
\end{PipsProp}
...
To make it work, you can set \verb+EOLE_OPTIMIZATION_STRATEGY+ to \verb+"CSE"+ or \verb+"ICM"+
This will only activate common subexpressions eliminationor invariant code motion.
...
alias invariant_code_motion 'Invariant Code Motion'
...
%%@UserManualDocumentation: inlining
...
%%@UserManualDocumentation: inlining_simple
...
%%@UserManualDocumentation: unfolding
...
%%@UserManualDocumentation: unfolding_simple
...
\index{UNFOLDING\_CALLEES}
Using \lstinline|UNFOLDING_CALLEES|, you can specify which module you want to inline in the unfolded module.
...
\index{UNFOLDING\_FILTER}
The default behavior of unfolding is to recursively inline all callees from given module, as long as a callee remains. Use \lstinline|UNFOLDING_FILTER| to inline all calls not in the space separated module list
\begin{PipsProp}
UNFOLDING_FILTER ""
\end{PipsProp}
...
%%@UserManualDocumentation: outline
...
The property \verb+OUTLINE_MODULE_NAME+ is used as new module name, and
the user is prompted if not set.

If set, property \verb+OUTLINE_LABEL+ is used to choose the statement to
outline.

The property \verb+OUTLINE_ALLOW_GLOBALS+ controls whether global
...
%%@UserManualDocumentation: clone_only
...
%%@UserManualDocumentation: clean_dynamic_variables
...
The transformation \verb/flatten_code/ firstly moves declarations up
in the abstract syntax tree, secondly remove useless braces and
thirdly fully unroll loops when there iteration counts are known.
...
\subsection{Split Initializations}

This very simple transformation recurses through all variable
declarations, and splits them in two statements if they specify an
initial value: a declaration without initialization, and an assignment
of the initial value.

The purpose of this transformation is to obtain correct use-def chains
and dependence graphs, because PIPS does not (yet) take into account
memory effects linked to declarations. This transformation transfers
these effects into usual executable statements.

This transformation can be used, for instance, to improve reduction
detection (see TRAC ticket 181).

Note that C array initializations cannot be converted into
assignments. In such cases, the initial declaration is left untouched.
...
%%@UserManualDocumentation: localize_declaration
...
%%@UserManualDocumentation: scalar_expansion
...
names are requested from the PIPS user?}
%%!UserManualDocumentation
...
%%@UserManualDocumentation: OMP Pragma
This transformation reads the sequential code and generates OpenMP pragma as
an extension to statements. The pragmas produced are based on the information
previously computed by differents phases and already stores in the pips internal
representation of the sequential code. It might be interesting to use the phase
internalize\_parallel\_code (see \S~\ref{sec:cons-parall-code}) before to apply
ompify\_code in order to maximize the number of parallel information available.
%%!UserManualDocumentation
...
The next property, \verb/PRETTYPRINT_IO_EFFECTS/, is used to control
...
%%@UserManualDocumentation: print_parallelizedOMP_code
...
some variables.\marginpar{RK: which one? FI: user\_request? Not found
in validation suite.}
%%!UserManualDocumentation
...
%%@UserManualDocumentation: print_code_smalltalk
...
%%@UserManualDocumentation: print_xml_code
...
%%@UserManualDocumentation: print_xml_code_with_explicit_motif
...
%%@UserManualDocumentation: print_claire_code
...
%%@UserManualDocumentation: print_claire_code_with_explicit_motif
...
%%@UserManualDocumentation: print_xml_application
