...
topic\marginpar{Serge? Still ture? What do you mean?}.
...
\lstinline|int| or \lstinline|int*| depending on the read/write
effect. 

If you want to pass the upper bound expression of a particular loop as
a parameter, which is used in Ter@pix code generation, set
\PipsPropRef{OUTLINE_LOOP_BOUND_AS_PARAMETER} to the loop label.

The property \PipsPropRef{OUTLINE_MODULE_NAME} is used to specify the new module
name, and the user is prompted if it not set.  

But first the pass scans
the code for any statement flagged with the pragma defined in
\PipsPropRef{OUTLINE_PRAGMA}.

If set, property \PipsPropRef{OUTLINE_LABEL} is used to choose the
statement to outline.
...
not. Normally, this should be addressed by a previous privatization pass.

Finally, property \PipsPropRef{OUTLINE_INDEPENDENT_COMPILATION_UNIT}
make it possible to outline into a newly created compilation unit,
adding all necessary types to this new compilation unit, named after
the \PipsPropRef{OUTLINE_MODULE_NAME}.
