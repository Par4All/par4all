...
\usepackage{epsfig,amstext,alltt}
...
In C, the initializer can generate directly a \verb|c_source_file|.
...
A prettyprint of the symbol table for a Fortran module is generated with:
\begin{verbatim}
...
A C file is a compilation unit with macros, include statements, local
and global variable definitions, type definitions, and with some or no
function definitions. When the PIPS workspace is created by PIPS
preprocessor, each C file is preprocessed\footnote{Macros are
interpreted and include files are expanded. The result depends on the
C preprocessor used,on its option and on the system environment
(/usr/include,...).} using for instance
\texttt{gcc -E}\footnote{It can be redefined using
\texttt{CPP\_PIPS\_ENV} and \texttt{CPP\ PIPS\_OPTIONS\_ENV} environment
variables} and broken into a new compilation unit which contains only
variable and function and type definitions and one C file for each C
function defined in the initial C file.

The compilation unit does not contain any function definitions after
the\marginpar{RK: gnih? to be deleted (FI)} workspace creation.

The new compilation units must be parsed before the new files
containing each one function definition can be parsed. The new
compilation units are named like the initial file name with a bang
extension. For instance, file \texttt{foo.c} will lead to a new
\texttt{foo!.c} compilation unit. This new compilation unit may be empty.
...
parsing of the new compilation unit before the parsing of its
associated functions. It is in fact a hash table containing C keywords
and typedef names in each compilation unit.\marginpar{RK: mais de quoi ça
cause ? (FI: to be deleted?)}
...
If you want to parse some C code using \texttt{tpips}, it is necessary to select the C
parser with
\begin{alltt}
activate C_PARSER
\end{alltt}
...
Gen} and {\em Kill} sets, but PIPS used the more general concept of
...
Fortran parallel DO loops. Interprocedurally, memory effects on
dynamic (a.k.a. stack allocated) variables are ignored in summary
effects because they cannot be observed by the callers\footnote{Unless
it accesses illegally the stack: see Tom Reps,
http://pages.cs.wisc.edu/~reps}.
\marginpar{RK: Not clear (FI: To be deleted)}
...
Cumulated effects of a all module are used to compute the proper
effects of the corresponding call sites. They are translated from the
callee's scope into the caller's scope. The translation is based on
the actual-to-formal binding. If too many actual arguments are
defined, a user warning is issued but the processing goes on because a
simple semantics is available: ignore useless actual arguments. If too
few actual arguments are provided, a user error is issued because the
effects of the call are not defined. It would be necessary to assume
either a general \verb+READ+ or a general \verb+WRITE+ for each read
or written formal parameter with no actual counterpart\marginpar{This
is implemented for calls by reference as in Fortran. The call-by-value
...
of June 21, 2008 (FI).}
\marginpar{RK: What are they? (FI: To be deleted)}
...
been added by Béatrice \textsc{Creusillet} to improve speed. The other tests could
...
implemented in \PIPS{}. \marginpar{Not clear}
...
\marginpar{The following is hard to read without any example for someone
  that knows nothing about PIPS...}
...
in fact computed along with the transformers (François \textsc{Irigoin}, 4 Sept. 90).
%%!UserManualDocumentation
...
by Rémi \textsc{Triolet} and then revisited by François \textsc{Irigoin}. The concept of
region is being revisited by Béatrice \textsc{Creusillet}, who added \verb+MUST+,
\verb+IN+ and \verb+OUT+ regions. Alexis \textsc{Platonoff} implemented the first
version of region analysis in \PIPS{}. These regions were computed with
respect to the initial stores of the modules. François \textsc{Irigoin} and,
mainly, Béatrice \textsc{Creusillet} added new functionalities to this first
...
A new type of array regions, from Manjunathaiah \textsc{Muniyappa}.
\marginpar{Sounds great ! :-)}
...
(see Section~\ref{sec:scalar-and-array-privatization}). A
...
\subsection{Considering a parallel code as a sequential one}
...
improving the parallelized code with some other transformations (dead-code
elimination, etc) is also useful. Thus we add a new pseudo-transformation
that coerce a parallel code into a classical (sequential) one to consider
parallelization as an internal transformation in PIPS.
...
parallel one if the good properties are not switched. Anyway, it's here.
...
it!\marginpar{Who is working on ? :-/}
...
nobody should have to require intermediate targets such as ???. Ya!\marginpar{hmmm}
...
\marginpar{IT SudParis : include your work here}
...
PHRASE-pragmas (written by the programmer) and a control file managing 
them. Those portions of code are intented, after transformations, to 
be executed in reconfigurable logic (in PHRASE projects, reconfigurable 
logic is synthetized with smalltalk code).
...
This phase is a preparation phase for (Phrase Distributor or
PHRASE\_DISTRIBUTOR): the portions of code to externalize are identified
and isolated here. Comments are modified by this phase.
%%!UserManualDocumentation
...
Initialisation or PHRASE\_DISTRIBUTOR\_INIT). The job of distribution is
done here.
%%!UserManualDocumentation
...
\marginpar{Ronan : include Mathieu Godet's work}
...
The \texttt{spaghettifier} is used in the context of the PHRASE project while
creating "Finite State Machine"-like code portions in order to synthesize
them in reconfigurables units.

This phases transforms structured code portions (eg. loops) in
unstructured statements.

The \texttt{SPAGHETTIFIER} transforms the module in a unstructured code with hierarchical unstructured portion of code corresponding to old strutures.
...
Where as the \texttt{SPAGHETTIFIER} transforms the module in a unstructured
code with hierarchical unstructured portion of code corresponding to
old strutures, the \texttt{FULL\_SPAGHETTIFIER} transform the code in a SEQUENCE
statement with a beginning statement, a UNIQUE and FLATTENIZED
...
In fact, this phase is the succession of the 2 phases
\texttt{FULL\_SPAGHETTIFY} (\S~ref{paragraph-full-spaghettify}) and
\texttt{FSM\_GENERATION} (\S~\ref{paragraph-fsm-generation})
...
given by the user are frozen.\marginpar{? Does it depends of the global
  climate warming?}
%%!UserManualDocumentation
...
User assisted cloning.\marginpar{terse}
%%!UserManualDocumentation
\begin{verbatim}
...
\subsection{Initial Preconditions}
...
\subsection{Sequential View with Proper Regions}
...
\subsection{Sequential View with IN Regions}
...
\subsection{Sequential View with OUT Regions}
...
\subsection{Sequential View with Privatized Regions}
...
decoration. \PIPSMAKE{} would deduce the other ones. There is no need
for a transitive closure, but some people like it that way to make
resource usage verification possible...\marginpar{explain...}
...
\section{uDrawGraph Interprocedural Control Flow Graph Files (DVICFG)}
...
Same as \texttt{print\_whole\_dependence\_graph} but it's filtered by some
variables.\marginpar{which one?}
%%!UserManualDocumentation
...
Check impact of alias on the dependance graph.\marginpar{What is that?}
%%!UserManualDocumentation
...
be viewed as a SmallTalk pretty-printer of a subset of Fortran.
%%!UserManualDocumentation
...
This pass generates XML code like the \textsc{print\_xml\_code} pass, but
the input contains explicitly loops to scan motifs.\marginpar{gnih?}
%%!UserManualDocumentation
...
This pass is used in the DREAM-UP project for specification input and
transformation. This function can be viewed as a CLAIRE pretty-printer of
a subset of Fortran.
%%!UserManualDocumentation
...
This pass generates CLAIRE code like the \textsc{print\_claire\_code} pass, but the input contains explicitly loops to scan motifs.
%%!UserManualDocumentation
...
some typing issues in the code. To be fixed...
