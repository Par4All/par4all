...
Added two properties for R-Stream compatibility.
If \PipsPropRef{OUTLINE_REMOVE_VARIABLE_RSTREAM_IMAGE} is set to True, instead of outlining
loop indexes and written scalar variables the outliner declares them as local variables inside the
outlined function. Therefore they are not added to the effective or formal parameters.

The behavior of \PipsPropRef{OUTLINE_REMOVE_VARIABLE_RSTREAM_SCOP} is the same as above. Except that this property
only excludes loop indexes from being outlined. 
