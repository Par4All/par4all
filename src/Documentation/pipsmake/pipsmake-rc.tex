...
The \{PipsPropRef} property can be used to extend the information available
for varibales. By default the entity name, the offset and the size are printed.
Using this property the type and the user name are added.
...
Some phases use \PipsPropRef{ACCEL_LOAD} and \PipsPropRef{ACCEL_STORE} to generate DMA calls
...
KERNEL_LOAD_STORE_LOAD_FUNCTION_2D "P4A_copy_to_accel2d"
\end{PipsProp}
...
KERNEL_LOAD_STORE_STORE_FUNCTION_2D "P4A_copy_from_accel2d"
\end{PipsProp}
...
Note that since function prototypes are also changed accordingly, it is
more general than a pure expression transformation.
