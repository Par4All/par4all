...
Property \PipsPropRef{FUSE_MAXIMISE_PARALLELISM} is used to control if loop fusion
has to be care about maximizing parallelism while fusing. This means that we won't
fuse a loop marked as parallel with a sequential loop. 

\begin{PipsProp}{FUSE_MAXIMISE_PARALLELISM}
FUSION_MAXIMIZE_PARALLELISM TRUE
\end{PipsProp}
