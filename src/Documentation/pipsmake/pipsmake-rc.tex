...
\begin{PipsPass}{loop_expansion}
  ...
  set by \PipsPropRef{LOOP_EXPANSION_OFFSET} to accuratly change the
  lower bound too.  The new loop is guarded to prevent illegal
...
\begin{PipsPass}{loop_fusion}
  ...
  bounds and if possible the same loop indices. If n loops can be
  potentially fused, the fusion of first two ones is attempted first,
  and then more fusions are attempted two. So if a sequence of three
  loops is processed and if Loop 2 can be fused either with Loop 1 or
  with Loop3 but not with both, Loop 2 is fused with Loop 1 and Loop 3
  is left aside.
...
Property \PipsPropRef{FUSE_MAXIMISE_PARALLELISM} is used to control if
