...
{\tt proper\_effects\_with\_points\_to} is an alternative to compute
constant path proper effects using points-to analysis (see
subsection~\ref{subsection-points-to-analysis}). This is still highly
experimental.
...
\begin{PipsProp}{USER_EFFECTS_ON_STD_FILES FALSE}
...
access paths cannot point to the same memory locations. The safe (and
...
\subsection{Manage Parallel Loops}
\label{sec:manage-parallel-loops}

\begin{PipsPass}{manage_nested_parallelism}
The following transformation works on the parallel information available
in the sequential code. The transformation will limit the number of nested
parallel loops to a given value.
It might be interesting to use the phase
internalize\_parallel\_code (see \S~\ref{sec:cons-parall-code}) before to apply
manage\_nested\_parallelism in order to maximize the number of parallel
information available.
\end{PipsPass}

\begin{PipsMake}
manage_nested_parallelism          > MODULE.code
        < MODULE.code
\end{PipsMake}

PIPS relies on the property NESTED\_PARALLELISM\_THRESHOLD to determine the
number of nested parallelism to generate.
...
Transfer are generated using either loops or \PipsPropRef{ISOLATE_STATEMENT_LOAD_FUNCTION} and \PipsPropRef{ISOLATE_STATEMENT_STORE_FUNCTION}
\end{PipsPass}
\begin{PipsMake}
...
\begin{PipsProp}{ISOLATE_STATEMENT_LOAD_FUNCTION}
ISOLATE_STATEMENT_LOAD_FUNCTION ""
\end{PipsProp}

\begin{PipsProp}{ISOLATE_STATEMENT_STORE_FUNCTION}
...
force the load of any array that is writed in the kernel. This actually
fixes the problem of partially writed array. A better solution using the region
is under implementation.
\begin{PipsProp}{KERNEL_LOAD_STORE_FORCE_LOAD}
KERNEL_LOAD_STORE_FORCE_LOAD FALSE
\end{PipsProp}
...
hardware accelerator, such as SPoC and Terapix.
...
(size of which is the number of pe):
...
If  \PipsPropRef{PARTIAL_EVAL_LINEARIZE} is set to TRUE,
Additionnal simplifications are done
\begin{PipsProp}{PARTIAL_EVAL_LINEARIZE}
PARTIAL_EVAL_LINEARIZE FALSE
\end{PipsProp}
...
Set \PipsPropRef{RELAX_FLOAT_COMMUTATIVITY} to TRUE if you wan to consider some floating point operation as commutative
\begin{PipsProp}{RELAX_FLOAT_COMMUTATIVITY}
RELAX_FLOAT_COMMUTATIVITY FALSE
\end{PipsProp}
...
If \lstinline|POINTER| is chosen, call sites are modified to keep proper signature.
...
Compiler tends to produce many parallel loops whcih is genrally not optimal for
performance. The following transformation merges nested omp pragma in a unique
omp pragma.
\end{PipsPass}
...
PIPS merges the omp pragma on the inner or outer loop depending on the property
OMP\_MERGE\_POLICY. The property can be set to \verb|outer| or \verb|inner|
\begin{PipsProp}{OMP_MERGE_POLICY}
...
it only changee the program perfomances.
...
OMP\_LOOP\_PARALLEL\_THRESHOLD\_SET. To summarize, rmenber that the property
