...
subspace (i.e. \emph{à la} K{\sc arr}~\cite{Kar76}) or one based on
...
transformations like \PipsPassRef{partial_eval}, \PipsPassRef{simplify_control} and \PipsPassRef{dead_code_elimination}.
...
by Rémi \textsc{Triolet}~\cite{Tri84} and then revisited by François
...
\begin{PipsPass}{loop_nest_unswitching}
  ...
  \PipsPassRef{simplify_control} works afterward
\end{PipsPass}
...
\begin{PipsPass}{directive_filter_c}
The \PipsPassRef{directive_filter_c} phase preproces an original
source file just as \PipsPassRef{directive_filter} but for C file.
\end{PipsPass}

\begin{PipsMake}
directive_filter_c          > MODULE.directive_filtered_file
   < MODULE.c_source_file
   < COMPILATION_UNIT.declarations
\end{PipsMake}
...
compute SEND and RECV regions leading to MPI messages.
\end{PipsPass}
...
\begin{PipsPass}{step_analyse_com}
The \PipsPassRef{step_analyse_com} phase checks whether a given SEND
region corresponding to a directive module is consumed by a RECV
region corresponding to a directive module. In this case,
communications can be optimized.
\end{PipsPass}
...
\begin{PipsMake}
step_analyse_com2            > PROGRAM.step_analyses
                             > MODULE.step_analyses_com
   < PROGRAM.entities
   < PROGRAM.directives
   < PROGRAM.step_analyses
   < CALLEES.step_analyses_com
   < MODULE.callees
   < MODULE.code
   < MODULE.dg
\end{PipsMake}
...
The property \PipsPropRef{STEP_RUNTIME} define which runtime will be provided
...
The property \PipsPropRef{STEP_OPTIMIZED_COMMUNICATIONS} set to
\texttt{FALSE} by \PipsPassRef{step_analyse} and set to \texttt{TRUE}
by \PipsPassRef{step_analyse_com}

\begin{PipsProp}{STEP_OPTIMIZED_COMMUNICATIONS}
STEP_OPTIMIZED_COMMUNICATIONS TRUE
\end{PipsProp}
...
\PipsPassRef{simplify_control} and \PipsPassRef{dead_code_elimination} after this
...
\end{PipsPass}
It is required to set \PipsPropRef{MASK_EFFECTS_ON_PRIVATE_VARIABLES} to
FALSE when computing effects needed by this pass.
...
Generate code for GPGPU, possibly through an OpenCL programmed library
implementing the FREIA AIPO functions.
...
If set to a non-zero value, optimize the imagelet size (height) with respect
to this size of image, so as to minimize the border effects. This should
not be needed if the runtime could adjust the computed size within the
imagelet.
\begin{PipsProp}{HWAC_TERAPIX_IMAGE_HEIGHT}
HWAC_TERAPIX_IMAGE_HEIGHT 0
\end{PipsProp}
...
the empty string, \verb/""/, the scalarized variable name is used as prefix. 
