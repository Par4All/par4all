...
Remove useless load store calls
\end{PipsPass}
...
The goal of Bufferization is to generate a dataflow communication through buffers between modules. The communication is done by special function call generate by \PipsPassRef{KERNEL_LOAD_STORE} with a special property. To keep flows consistant outside the module SCALOPIFY surround variable call with a special function too.
...
\begin{PipsPass}{kernel_load_store_fine_grain}
Same as \PipsPassRef{kernel_load_store} but region-based.
\end{PipsPass}
\begin{PipsMake}
...
Force loop fusion without any check between loop \PipsPropRef{LOOP_LABEL} and following loop,
that must share an iteration space with it.
\end{PipsPass}
\begin{PipsMake}
