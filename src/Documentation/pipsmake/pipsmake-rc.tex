...
Property \PipsPropRef{ATOMIC_CHAINS_FILTER_SCOPE} is used to filter out 
arcs based on variable scope. It ensure that if a variable is limited to a 
scope, no dependence arc involving this variable can escape from the scope.

\begin{PipsProp}{ATOMIC_CHAINS_FILTER_SCOPE} 
ATOMIC_CHAINS_FILTER_SCOPE TRUE
\end{PipsProp}
...
\verb/printf/ to force an OUT region.
...
\begin{PipsPass}{array_privatizer}
  ...
  only.  The algorithm, developed by B\'eatrice C{\sc reusillet}~\cite{C96}, is
...
% Ngu02
\bibitem{Nguyen2002} Thi Viet Nga Nguyen, {\em Efficient and
