...
\begin{document}
\svnInfo $Id: effects.tex 17253 2010-05-31 08:43:19Z creusillet $
\maketitle
\sloppy
\newpage
\tableofcontents
...
For instance $\mathtt{p[i]([0][.next])^(c)[.tab][j]}$ represents the
...
of effects. For historical reasons, exact effects are labelled
\texttt{MUST} and must effects are not computed because with
polyhedral sets, they are either equal to the exact set or to the
empty set.

\texttt{READ} and \texttt{write} effects are useful for dependence
analyses. However, it is sometimes interesting to see compound
statements as black boxes. In this case we only want to known if the
\emph{initial} value of a memory location is \emph{used} by the
statement, which is called a \texttt{IN} effect, or if the memory
location is only used for temporary storage. In the same way, it is
important to know if the value left by a statement in a memory
location is dead when leaving a statement or if it is used later by
another statement execution. In the later case, it is called an
\texttt{OUT} effect. Currently \texttt{IN} effects are internally
represented as \texttt{READ} effects, and \texttt{OUT} effects as
\texttt{WRITE} effects. The programmer must be aware of the type of
effects he is dealing with.
