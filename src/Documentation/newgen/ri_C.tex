...
In this framework, the effect of a statement like \verb/*p=1;/ can
 only be expressed in this framework as a reference to a large memory
...
We do not know if we need to keep track of pointer arrays, as in
 ...
 containing pointers ot functions.
...
The last solution, Solution~\ref{new-effect}, would require a proper
...
In the short term, the best solution is to add a new field
...
All accesses are indexed by default in PIPS internal representation,
 so \verb/post/ and \verb/pre/ are equivalent for scalar accesses.

Mode \verb/index/ could be called \verb/direct_indexing/ to be more
 homogeneous, but indexing always occurs. So \verb/index/ could be
 called \verb/direct/.

The mode \verb/preindexing/ does not seem to be very useful in the
 short term.
