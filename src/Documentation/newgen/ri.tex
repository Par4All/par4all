...
\title{PIPS: Repre'sentation Interme'diaire}
...
\section{Entite's: variables, fonctions, e'tiquettes}
\label{entity}
...
{
Le domain \verb/unstructured/ permet de repre'senter des instructions 
non-structure'es sous forme de graphe structure'. Le graphe est dans le
champ \verb/control/ tandis que \verb/exit/ correspond au noeud de
sortie du graphe (il ne doit pas e^tre modifie' par un utilisateur).
}
...
{ 
Le domain \verb/control/ est utilise' pour imple'menter le graphe de
contro^le structure' d'un \verb/statement/. Dans un tel graphe, tous les
statements structure's sont conserve's; les branchements ont e'te'
e'limine's et remplace's par des arcs du graphe de contro^le. Ce graphe
est structure' car si un branchement est local (par exemple a` un corps
de boucle) alors, pour les instructions englobantes, le statement
correspondant sera vu comme structure'; ceci explique la re'cursion
mutuelle de \verb/control/ et \verb/statement/ (via \verb/instruction/). 

Tous les statements, sauf
\verb/test/, ont un seul successeur; le premier successeur de
\verb/test/ correspond au cas ou` la condition est ve'rifie'e (le second
au cas contraire).
}
