...
%% $Date: 1996/11/16 19:02:22 $, 

\documentstyle[a4,psfig,html,verbatim,newgen_domain]{article}
...
understanding of Newgen declarations\footnote{If you edit this document,
please remember that NewGen declarations introduced by \verb/\domain/
must fit on one line.} is assumed throughout this document, but the
notation is close enough to standard programming language to be
interpreted by newcommers. For more information about NewGen see
\cite{JT89}\cite{JT90}.

This document is part of PIPS documentation and its Latex version is
located in \verb+$PIPS_ROOT/Src/Documentation/newgen/ri.tex+ (\verb+ri+
...
between types is used to build the internal representation, but they are
...
string and a special one-character separator, \verb/MODULE_SEP_STRING/. 
The prefix string is the name of the package
...
in/out information. Field {\tt result} contains the result type. Type
...
or by the semantics analysis\footnote{Béatrice: How about $\phi$
...
\verb+STATEMENT_NUMBER_UNDEFINED+. This number may be set explictly by
...
Flow Graph (HCFG) used by PIPS. See Section~\ref{subsubsection-unstructured} for information about
...
default value is \verb+STATEMENT_ORDERING_UNDEFINED+.
...
flow graph, whereas NewGen iterators
...
special value, \verb/string_undefined/. Comments associated to
...
operators. There are {\tt call}'s for assignments, subroutine calls,
input-outputs, returns, stops, modulos, the overloaded {\tt +}, and so
on. This is detailed in Section~\ref{subsubsection-call}.
...
control restructurer but it is interfaced to Toolpack.}. The user-defined form only is
used by the parser and some pretty-printers. The pretty-printers are
able to restore Fortran-77 {\tt goto}'s from the {\tt
unstructured}. More on this in Section~\ref{subsubsection-unstructured}.
...
no effect, a NOP. See {\tt empty\_statement\_p()}.

\subsubsection{Control Flow Graph (a.k.a. Unstructured)}
\label{subsubsection-unstructured}
\index{Unstructured}\index{HCFG}\index{Control Flow Graph}
...
concurrently. Entities in the {\tt local} field are loop-private
...
such as assignment {\tt =},{\tt READ, WRITE, RETURN, CALL}, are all
encoded in the same way, like user-defined function calls. The number of
arguments depends on the pseudo-function: 0 for constants, 1 or 2 for
operators, and so on. Fortran keywords, operators and intrinsics are
known as predefined functions. This unification of language and user
defined functions is useful to reduce the size of the datastructure
...
Type \verb/syntax/ is used to represent expressions as they are defined
in the program source code. A \verb/syntax/ object is either a
...
explains why the \verb/call/ type is defined in the {\em Instruction}
section.}, or a \verb/range/ as in loop definitions\footnote{This
explains why type \verb/range/ is defined in the {\em Instruction}
section.} and array declarations.
...
Effects and the types defined in the following
subsections are not used to represent code, but to store analysis
result. These types are declared in the {\em internal representation}
...
do not use the last field, \verb/context/.
...
as regions, which use pseudo-references based on $PHI$ variables. Memory
...
and field \verb/approximation/ is used to know if the reference, for
simple effects, or the set of array elements defined by the
\verb/context/, for array regions, is or not accessed for sure. For
instance, a conditional is going to generate may effects, and a
sequence, must effects.
...
\end{quote}
Unfortunately, thie name depends on the module parsing order. It would
...
as well as pseudo-variables, known as $PHI$ variables. There is one
$PHI$ variable per array dimension.
...
regions}. They were defined by R\'emi Triolet in~\cite{Trio84}.
...
Sometimes, a simple statement, such as an assignment like {\tt I = 2},
has a known effect. Here {\tt I} \verb/must/ be written. Such
...
specific to the semantics library. The final values are indentified with
...
variables and other integer entities such as $PHI$ variables (see
...
{\em Construction flexible et coh\'erente pour la compilation
interproce'durale}, 
...
{\em PIPSMAKE and PIPSDBM: Motivations et fonctionalit\'es},
Rapport Interne CAII TR~E/133

\bibitem{Trio84} R. Triolet,
{\em Contribution \`a la parall\'elisation autoamtique de programmes
Fortran comportant des appels de proc\'edures}, Th\`ese de
Docteur-Ing\'enieur, Universit\'e Pierre et Marie Curie, d\'ecembre 1984.
