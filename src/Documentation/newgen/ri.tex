...
stands for Internal Representation, \emph{représentation interne} in
...
according to the \emph{main} type in their signatures. There is no partial order
...
ou \texttt{3I == J}. Ces contraintes sont utilisées dans les syste`mes
...
appear here for the semantics analysis and are used by the 
\texttt{predicate} type (see Section~\ref{semantics}).

\begin{comment}
Le domaine \texttt{Psysteme} est utilisé pour représenter les syste`mes
...
intrinsic\index{Intrinsic}, a constant, a label,... Field \texttt{name}
...
\begin{comment}
Le domaine \texttt{callees} sert a` porter des informations
...
comme + et *, les intrinse`ques comme \texttt{MOD} ou \texttt{SIN} et surtout
...
chose que des références a` des scalaires entiers (ex: \texttt{T(I-1) +
T(I) + T(I+1)}).
\end{comment}
...
tableau, a lieu un effet. L'attribut \texttt{persistant} lui est donne'
...
\begin{comment}
Le sous-domaine \texttt{action} pre'cise si la
re'fe'rence est lue ou e'crite, et le sous-domaine \texttt{approximation}
...
\begin{comment}
Le sous-domaine \texttt{reference} permet de pre'ciser qu'un effet n'a lieu que
...
\begin{comment}
Le sous-domaine \texttt{context} n'est utilise' que pour repre'senter les
...
d'un \texttt{statement}. Il se peut même que certaines exe'cutions
y acce`dent et que d'autres n'y fassent pas re'fe'rence. Les effets
calcule's sont alors de type \texttt{may}.
\end{comment}
...
l'effet est certain (\emph{must}). Il peut alors être utilise' dans
...
\begin{comment}
Le domaine \texttt{transformer} de'finit une relation entre deux e'tats
...
aussi associe' a` un \texttt{statement}, donne une relation entre l'e'tat
initial d'un module et l'e'tat pre'ce'dent l'exe'cution de ce \texttt{statement}.
\end{comment}
...
\begin{comment}
Le domaine \texttt{predicate} de'finit une relation entre valeurs de
...
abstraction d'une commande, c'est-a`-dire d'un \texttt{transformer}.
\end{comment}
