...
\texttt{REAL} or \texttt{INTEGER}. Each member includes a precision
information, which can be used to derive the number of bytes or bits
required to store one scalar object of this type. The precision
information is numerically known for most basic types but not for
\texttt{overloaded}, \texttt{string}, \texttt{bit}, \texttt{derived},
\texttt{pointer}. Not that for some C \texttt{int} types, the
information \texttt{int:int} must be fixed.
...
\domain{Parameter = type x mode x dummy:string}

Objects of type \texttt{parameter} represents type and inout
information for formal parameters. The dummy field is used to store a
parameter name in C function declarations. By default, this name is
the empty string, "".
...
\domain{Constant = int + float:int + logical:int + litteral:unit + call:entity + unknown:unit}
{}
