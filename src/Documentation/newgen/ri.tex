...
\domain{Effect = cell x action x addressing x approximation x descriptor}
{}
...
either a \verb/reference/ or a persistant\footnote{Persitant is a
  Newgen attribute carried by a typ or by a field. It means that
  recursive Newgen procedures must stop there. This impacts especially
  the free and copy functions.} \verb/preference/. Attribute
\verb/persistant/ is used because reference objects point to may be
...
Field \verb/addressing/ specifies the addressing mode. The addressing
can be direct and indexed, as in \verb/x[i]/, indirect and
post-indexed, as in \verb/(*p)[i]/, indirect and pre-indexed, as sin
\verb/*(p[i])/. For Fortran code, indirect addressing modes are not
used.
...
\subsection{Addressing Modes of an Effect}
\label{subsection-access}
\index{Access}

\domain{Addressing = index:unit + pre:unit + post:unit}
{}

Three addressing modes are used. The default addressing mode is the index
mode. Scalars are accessed like arrays of 0 dimension. The index mode
is the only mode used for Fortran programs. The indirect pre-indexing
mode is used to represent references such as \verb/*(p[i])/. The
indirect popst-indexing mode is used to represent references such as
\verb/(*p)[i]/.
