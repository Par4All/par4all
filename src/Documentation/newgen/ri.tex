...
\verb+Documentation/newgen/ri.tex+ of the sources (\verb+ri+ stands for {\em
représentation interne} in French). It is used to generate a PIPS
include file located in \verb+$PIPS_ROOT/Include/ri.h+. The functions
available to manipulate these data structures are grouped in
\verb+Lib/ri-util+ of the sources. These functions are grouped in files
...
or synthesized, the field \verb/decls_text/ is destroyed and set to ???
...
par defaut: \verb+STATEMENT\_NUMBER\_UNDEFINED+). Ce numéro est défini
...
\verb+STATEMENT\_ORDERING\_UNDEFINED+. Il est automatiquement recalculé
...
prend la valeur \verb/string\_undefined/. Les commentaires associés a`
...
\domain{statement\_effects = statement->effects}
{}
