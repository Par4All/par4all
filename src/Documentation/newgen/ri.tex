...
\usepackage{epsfig,verbatim,comment,newgen_domain}
...
intrinsic\index{Intrinsic}, a constant, a label\ldots Field \texttt{name}
...
\texttt{area} is used for commons\index{Common}. Two additional areas are
defined as implicit common, the static and the dynamic areas associated
to a module. Their specific names are defined in \texttt{ri-util.h}.
...
Type \texttt{area} is used to represent storage sections for variables such
as commons or static or dynamic area. Areas are \emph{global} objects in
...
\domain{Basic = int:int + float:int + logical:int + overloaded:unit + complex:int + string:value + bit:int + pointer:type + derived:entity + typedef:entity}

Type \texttt{basic} is used to store basic type information such has 
\texttt{REAL} or \texttt{INTEGER}. Each member includes a precision information,
...
Note that no \emph{default} type is provided. Untyped object are given
...
that \texttt{IMPLICIT} statements should appear as early as possible in a
module declarations.
...
\domain{Qualifier = const:unit + restrict:unit + volatile:unit + register:unit + auto:unit}
...
Objects of type \texttt{parameter} represents type and inout information
...
Type \texttt{mode} is used to carry inout information for formal
parameters. Member \texttt{value} is used for calling by value. Member 
\texttt{reference} is used for calling by reference. Fortran uses calls by reference.
...
Member \texttt{return} is appropriate for Fortran functions. The value
...
thru the \texttt{return} field is the corresponding function.
...
Member \texttt{section} contains the \emph{area} in which the variable is
...
for local variables. Static variables are explicitly declared in a 
\texttt{SAVE} statement or implicitly made static by a \texttt{DATA}
statement\footnote{See Fortran standard Section~(8-11) about SAVE and
  Section~(9-1) about DATA.} unless they are explicitly declared in a 
  \texttt{COMMON} because PIPS sets all commons as static. 
...
donne l'adresse dans l'aire de la variable. Enfin, le sous-domaine 
\texttt{shared} donne la liste des entités qui partagent statiquement un
...
Member \texttt{expression} is used to restore user-frinedly
...
Type \texttt{code} is not used to stored module bodies. The effective code
body must be retrieved from the PIPS database through a call to 
\texttt{pipsdbm} (see \cite{Trio90}). There is no direct link
...
paramèters formels et commons déclarés dans la fonction.
\end{comment}
...
transformé, le prettyprinter régénère des déclarations
synthétiques.
\end{comment}
...
de convertir les appels à RETURN en des branchements à ce label
...
la première ligne de l'instruction dans le fichier source. Il n'est (en
principe) jamais modifié par PIPS. Après déroulage de boucle,
plusieurs \verb/statement/s peuvent avoir le même numéro.
\end{comment}
...
statement dans dans cette composante; cet entier sert à comparer
...
après chaque transformation de programme. 
...
interne et doit cond être calculée explicitement.
\end{comment}
...
accessibles d'après le flot de contrôle n'en ont pas. Les statements
sans étiquette se trouvant derrière un GOTO en sont un exemple. Les
statements peuvent être parcourus de manière sélective, suivant le
flot de contrôle, avec la macro \verb/CONTROL_MAP/\index{CONTROL MAP},
ou de manière systématique avec les fonctions de parcours de NewGen,
...
\begin{comment}
Le sous-domaine \texttt{comments} contient le texte du commentaire associé
a ce statement dans le programme initial; ce texte est utilisé par le
prettyprinter. Ce sont les commentaires qui précèdent le statement
qui s'y trouvent associés. En l'absence de commentaires, ce champ
prend la valeur \verb/string_undefined/. Les commentaires associés à
...
\domain{Instruction = sequence + test + loop + whileloop + goto:statement + call + unstructured + multitest + forloop + return:expression + expression}
...
loops. Field \texttt{index} points to the loop index, an entity. Field 
\texttt{range} contains the lower and upper bounds, as well as the step. Field
\texttt{body} points to the loop body, a unique statement which usually is
a sequence. Field \texttt{label} is used for Fortran labelled DO loops. It
is the label of the last statement in the loop body. Field \texttt{execution}
specifies if the loop should be executed sequentially or
concurrently. Entities in the \texttt{local}\index{Private} field 
are loop-private
...
sous-domaine \texttt{syntax} contient la description de l'expression telle
qu'elle apparaît dans le texte source du programme. Le sous-domaine
\texttt{normalized} contient une forme compilée des expressions
...
\begin{comment}
Si le sous-domaine \texttt{normalized} contient la valeur 
\texttt{normalized\_undefined}, cela signifie que la fonction de linéarisation
...
\domain{Syntax = reference + range + call + cast + sizeofexpression + subscript + application}
{}
...
domaine \texttt{call} est défini dans la section \emph{Instructions}), soit
un \verb/range/, dans le cas des expressions bornes de boucles (le domaine
\texttt{range} est présenté au niveau des boucles, aussi dans la section
\emph{Instructions}).
\end{comment}
...
due to the \emph{compound} nature of the PIPS \verb/DO/ construct. It if
...
The \texttt{effects} domain is a list of individual effects. Each effect
...
statements like \texttt{blocks}, \texttt{tests}, \texttt{loops} and 
\texttt{unstructured}. 
...
are used to build use-def chains, dependence
%% http://www.cri.ensmp.fr/pips/pipsmake-rc.html} 
...
le point de départ du calcul use-def chains, des
%%{http://www.cri.ensmp.fr/pips/pipsmake-rc.html} 
dépendances, des Summary Data Flow Information (known as summary
effects at the module level, and as cumulated effects at the statement
level), et des régions.
\end{comment}
...
\begin{comment}
Le sous-domaine \texttt{reference} indique sur quelle variable, scalaire ou
tableau, a lieu un effet. L'attribut \texttt{persistant} lui est donné
parce que les références concernées font directement partie des
statements du programme, au moins quand il s'agit d'effets simples. Cet
attribut est naturellement gênant pour les régions qui utilisent des
références synthétiques basées sur les pseudo-variables $PHI$.
\end{comment}
...
\begin{comment}
Le sous-domaine \texttt{action} précise si la
référence est lue ou écrite, et le sous-domaine \texttt{approximation}
permet de savoir si la référence (pour les effets) ou l'ensemble des
éléments de tableaux défini par le contexte (pour les régions) est lu ou
écrit à coup sûr ou non. 
\end{comment}
...
\begin{comment}
Lors de la traduction interprocédurale des effets ou des régions portant
sur une variable globale non déclarée dans le module appelant ou sur une
variable statique apparaissant dans un \texttt{SAVE}, il est important de
récupérer une référence qui sera la même pour tous les sites d'appels.
Pour cela on choisit un module de référence (dans lequel le common ou le
save est déclaré), dont le nom préfixera le nom de la variable. Le choix
de ce module n'est pas trivial. À l'heure actuelle, il s'agit du nom du
module de la première variable apparaissant dans la liste des variables du
common:
\begin{quote}
 \texttt{ram\_section(storage\_ram(entity\_storage(<my\_common>)))}
\end{quote}
Selon l'ordre dans lequel les modules sont analysés, le nom de module
trouvé sera différent. Il faudrait en réalité prendre le premier nom
de module dans l'ordre lexicographique des modules appelés (callees).
\end{comment}
...
\begin{comment}
Le sous-domaine \texttt{reference} permet de préciser qu'un effet n'a lieu que
sur un sous-tableau en utilisant un \texttt{range} comme expression d'indice.
Ceci est utilisé lors de la traduction des effets cumulés d'un procédure
en les effets propres d'un call site. Pour les régions, ce sous-domaine
précise l'entité concernée ainsi que la liste des variables $\phi$ qui
décrivent ses dimensions.
\end{comment}
...
\begin{comment}
Le sous-domaine \texttt{context} n'est utilisé que pour représenter les
effets des instructions par des régions (telles que définies par Rémi
Triolet).
\end{comment}
...
\begin{comment}
Deux types d'effets sont utilisés dans les conditions de Bernstein
et dans les conditions propres à chaque transformation de programme:
la lecture d'une variable et son écriture.
\end{comment}
