...
def conv(input,output):
	...
	def print_im(field): 		return print_fields(["im,"+field,' ',' ',' ',' '])
	def print_ma(field): 		return print_fields([' ',field,' ',' ',' '])
	def print_re(field): 		return print_fields([' ',' ',"P,"+field,' ',' '])
	...
	def ma(m,i):return m.group(i)
	
	
	#reX+=reY
	line=RE.sub(r'addi\(re([0-9]+), re([0-9]+)\)',
			lambda m:print_re(re(m,1)+'='+re(m,1)+'+'+re(m,2)),
			line)
	#reX*=reY
	...
	line=RE.sub(r'addi\(re([0-9]+), (im[0-9]+)\)',
			lambda m:print_re(re(m,1)+'='+re(m,1)+ +"+"+im(m,2)),
			line)
	...
	line=RE.sub(r'addi\(re([0-9]+), (im[0-9]+)\[re([0-9]+)\]\)',
			lambda m: print_re(im(m,2)+"+"+re(m,3))+print_re(re(m,1)+'='+re(m,1)+'+im'),
			line)
	#reX+=imY[\d+]
	line=RE.sub(r'addi\(re([0-9]+), (im[0-9]+)\[([0-9]+)\]\)',
			lambda m: print_re(im(m,2)+"+"+m.group(3))+print_re(re(m,1)+'+im'),
			line)
	#reX=imY
	line=RE.sub(r'seti\(re([0-9]+), (im[0-9]+)\)',
			lambda m:print_re(re(m,1)+"="+im(m,2)),
			line)
	#imX=imY
	...
	line=RE.sub(r'psetpi\((im[0-9]+), (im[0-9]+)\)',
			lambda m:print_re(im(m,1)+"="+im(m,2)),
			line)
	#*imX=*imY+*imZ
	line=RE.sub(r'addi\(\*(im[0-9]+), \*(im[0-9]+), \*(im[0-9]+)\)',
			lambda m:print_re(im(m,3)+"="+im(m,3))+print_re(im(m,1)+"=P+"+im(m,2)),
			line)
	#*imX=*imY+*maZ
	line=RE.sub(r'addi\(\*(im[0-9]+), \*(im[0-9]+), \*(ma[0-9]+)\)',
			lambda m:print_re(im(m,1)+"="+im(m,2)+"+"+ma(m,3)),
			line)
	#*imX=reY+*imZ
	line=RE.sub(r'addi\(\*(im[0-9]+), re([0-9]+), \*(im[0-9]+)\)',
			lambda m:print_re(im(m,3)+"="+im(m,3))+print_re(im(m,1)+"=P+"+re(m,2)),
			line)
	#reX=*imY+*imZ
	line=RE.sub(r'addi\(re([0-9]+), \*(im[0-9]+), \*(im[0-9]+)\)',
			lambda m:print_re(im(m,3)+"="+im(m,3))+print_re(re(m,1)+"=P+"+im(m,2)),
			line)
	#reX=reY+*imZ
	line=RE.sub(r'addi\(re([0-9]+), re([0-9]+), \*(im[0-9]+)\)',
			lambda m:print_re(im(m,3)+"="+im(m,3))+print_re(re(m,1)+"=P+"+re(m,2)),
			line)
	#reX=*imY+reZ
	line=RE.sub(r'addi\(re([0-9]+), \*(im[0-9]+), re([0-9]+)\)',
			lambda m:print_re(im(m,3)+"="+im(m,2))+print_re(re(m,1)+"=P+"+re(m,3)),
			line)
	#inX=inY+n
	line=RE.sub(r'paddi\((im[0-9]+), (im[0-9]+), ([0-9]+)\)',
		    lambda m:print_im(im(m,1)+"="+im(m,2)+"+"+(m.group(3) + "*" if m.group(3) != "1" else "" ) +"E"),
			line)
	#reX=re(Z)
	...
	line=RE.sub(r'seti\(re([0-9]+), ([0-9]+)\)',
			lambda m:print_re(re(m,1)+'='+m.group(2)),
			line)
	#maX=FIFOY
	line=RE.sub(r'seti\((ma[0-9]+), (FIFO[0-9]+)\)',
	...
	line=RE.sub(r'seti\((im[0-9]+)\[re([0-9]+)\], (re([0-9]+))\)',
			lambda m:print_im(im(m,1)+"+"+re(m,2))+print_im("im="+re(m,3)),
			line)
