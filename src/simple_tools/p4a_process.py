...
class p4a_processor(object):
    ...
    def gpuify(self, filter_select = None, filter_exclude = None):
        ...
        """
        all_modules = self.filter_modules(filter_select, filter_exclude)
        # This is some pre-optimization phases on the kernels
        # it may be optionally activated by a flag ... -On ?
        # scalarization helps isolate_statement not to generate
        # too much data transfers in some cases.
        self.workspace.props.ARRAY_PRIV_FALSE_DEP_ONLY=False
        all_modules.scalarization(force_out=True)
        # This is a workaround for bad p4a behavior in the presence
        # of declarations in a loop nest, something like
        # for(i ....) { int j;  for(j;....) ... }
        # the idea is to remove the inside declarations
        # using flatten code, then to make relevant variables locals
        # i.e. declared in the innermost loop (thus the skip_loop_indices)
        # and to remove unneeded declarations afterwards
        # be warned that p4a may still remove some defs in the post_processing step.
        try:all_modules.flatten_code(unroll=False)
        except:pass
        all_modules.localize_declaration(skip_loop_indices=True)
...
#        f_wrappers.print_call_graph ()
        ...
        if (self.fortran == False):
            # SG: too much confidence into linearize_array: apply it to all functions.
            # There should be a flag to choose here.
            ...
            self.workspace.all_functions.linearize_array(LINEARIZE_ARRAY_USE_POINTERS=True,LINEARIZE_ARRAY_CAST_AT_CALL_SITE=True)
        else:
