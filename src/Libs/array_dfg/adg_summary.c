...
graph                   dup_dg;
{
	...
	entry_q = make_quast( make_quast_value(is_quast_value_quast_leaf,
                  	...
                  	NIL );
	hash_put( Gvertex_number_to_statement, (char *) ENTRY_ORDER,
		 (char *) ENTRY_ORDER );
	...
	exit_q = make_quast( make_quast_value(is_quast_value_quast_leaf,
                  	...
                  	NIL );
	hash_put( Gvertex_number_to_statement,
		 (char *) EXIT_ORDER, (char *) EXIT_ORDER );
	...
	for(; !ENDP( write_list ); POP( write_list )) {
		...
		for(ie = 1; ie <= dims_length; ie++) {
			...
			ADD_ELEMENT_TO_LIST( dest_indic, EXPRESSION, 
		...
		for( ie = 1; !ENDP(dims); POP(dims), ie++) {
			...
			ind = adg_get_integer_entity( ie );
			pv = vect_substract( 
				EXPRESSION_PVECTEUR(dimension_lower( dim )),
		...
		for(; !ENDP( sou_l ); POP(sou_l) ) {
			...
			sou_stco =  (static_control) GET_STATEMENT_MAPPING( stco_map,
							  sou_s );
			 ...
			 */
			poss = adg_path_possible_source(
					&source, sou_v, sou_d, pa_full(), TAKE_LAST);
			...
			if (get_debug_level() > 2) { 
				fprintf(stderr, 
				  "\nPossible Source Statement (ordering %d) ",
				   sou_order);
			...
			for(ie = 1; !ENDP(sou_args); POP(sou_args), ie++){
			    ...
			    sou_e = copy_expression(
						    EXPRESSION(CAR(sou_args)));
			    pvec = vect_substract( exit_pv,
						  EXPRESSION_PVECTEUR( sou_e ));
			    if (pvec != NULL) {
				pss = sc_make( contrainte_make(pvec), 
			...
			/* Get predicate that comes from enclosing DO */
                        ...
                        if (prov_pr != predicate_undefined) {
                                prov_ps = sc_append( prov_ps,
					         predicate_system( prov_pr ) );
			}
			 ...
			 */
			sou_ps = adg_suppress_2nd_in_1st_ps( 
					sc_append(sou_ps, prov_ps), loc_context);
			...
			if (get_debug_level() > 4) {
				...
				if (loc_context != SC_RN) 
				  pu_vect_fprint(stderr,
						   loc_context->base);
			}
...
my_pip_count++;
			...
			adg_path_max_source(	local_source, &sou_q, 
						local_path, dest_psl, TAKE_LAST );
...
if (get_debug_level() > 0) fprint_dfg(stderr, make_graph(ret_verl));
	...
	for(; !ENDP( read_list ); POP( read_list )) {
		...
		for(; !ENDP( sou_l ); POP(sou_l) ) {
			 ...
			 */
			poss = adg_path_possible_source(
					&source, sou_v, sou_d, pa_full(), TAKE_FIRST);
			local_path = (Ppath) poss->pat;
			/* Not a possible source => get the next candidate */
			if (is_pa_empty_p( local_path )) continue;
			...
			for(; !ENDP(sou_read); POP(sou_read)) {
			  ...
			  */
			 sou_ps = adg_suppress_2nd_in_1st_ps( 
					 sc_append(sou_ps, sou_context), loc_context);
			 if ((sou_ps != NULL) && !my_sc_faisabilite( sou_ps )) continue;
...
my_pip_count++;
			 ...
			 /* Find the new source and simplify it */
			 adg_path_max_source(	local_source, &sou_q, 
						 local_path, dest_psl, TAKE_FIRST );
