...
y.tab.c: tokyacc.h gram.y
	...
	$(PARSE) -l yacc.in
	sed -e '/extern char \*malloc/d;s/YY/HH/g;s/yy/hh/g' y.tab.c | grep -v "#line" > s.tab.c
	mv s.tab.c y.tab.c
...
toklex.h: warning.h f77keywords hpfkeywords f77symboles
	cat f77keywords hpfkeywords f77symboles | nl -s: | cat warning.h - | sed "s/\([^:]*\):\(.*\)/#define TK_\2 \1/" > toklex.h

tokyacc.h: warning.h f77keywords hpfkeywords f77symboles
	cat f77keywords hpfkeywords f77symboles | nl -s: | cat warning.h - | sed "s/\([^:]*\):\(.*\)/%token TK_\2 \1/" > tokyacc.h
...
tar:
	...
	tar cvf hpfc.tar $(LIB_HEADERS) $(LIB_CFILES) Run RunHpfcTest \
		...
		fortran-run-time/*.f fortran-run-time/*.h fortran-run-time/Makefile* \
		fortran-run-time/*.c fortran-run-time/*.ref RunHpfcTest
	compress -v hpfc.tar
