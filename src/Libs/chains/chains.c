...
//bool effects_must_conflict_p( effect e1, effect e2 ) { return TRUE;}
...
static void local_print_statement_set( msg, s )
    ...
    SET_MAP( st, {
                fprintf(stderr, ",%p (%td) ",
                        st, statement_number( (statement)hash_get(effects2statement,st) ));
                  print_effect((effect)st);
            }, s );
    fprintf( stderr, "\n" );
}
...
static bool init_one_statement( st )
    ...
    if ( GEN( st ) == (set) HASH_UNDEFINED_VALUE ) {
        ...
        ifdebug(2) {
            fprintf( stderr, "Init statement %td with effects %p, ordering %tx\n", statement_number( st ), load_statement_effects( st ), statement_ordering(st) );
...
static void genkill_statement( s )
    ...
    ifdebug(2) {
        debug( 2, "genkill_statement", "Result genkill_one_statement for Statement %p [%s]:\n", s, statement_identification( s ) );
...
static void inout_loop( st, lo )
    ...
    set_union( DEF_IN( l ), GEN( st ), set_difference( diff, DEF_IN( st ), KILL( st ) ) );
    set_free( diff );
...
static void inout_whileloop( statement st, whileloop wl ) {
    ...
    set_union( DEF_IN( wlb ), GEN( st ), set_difference( diff, DEF_IN( st ), KILL( st ) ) );
    set_free( diff );
...
static void inout_forloop( statement st, forloop fl ) {
    ...
    set_union( DEF_IN( flb ), GEN( st ), set_difference( diff, DEF_IN( st ), KILL( st ) ) );
    set_free( diff );
...
static void inout_call( statement st, call __attribute__ ((unused)) c ) {
    ...
    set_union( DEF_OUT( st ), GEN( st ), set_difference( diff, DEF_IN( st ), KILL( st ) ) );
...
static void inout_statement( statement st ) {
    ...
    ifdebug(2) {
        fprintf( stderr, "%*s> Computing DEF_IN and OUT of statement %p (%td %td):\n", indent++, "", st, statement_number( st ) , statement_ordering( st ) );
    ...
    ifdebug(2) {
        fprintf( stderr, "%*s> Statement %p (%td %td):\n", indent--, "", st, statement_number( st ), statement_ordering( st ) );
        local_print_statement_set( "DEF_IN", DEF_IN( st ) );
        local_print_statement_set( "DEF_OUT", DEF_OUT( st ) );
        local_print_statement_set( "REF_IN", REF_IN( st ) );
        local_print_statement_set( "REF_OUT", REF_OUT( st ) );
...
static cons *pushnew_conflict( fin, fout, cfs )
    ...
    ifdebug(2) {
        fprintf( stderr, "Adding %s->%s\n", entity_name( effect_entity( fin )), entity_name( effect_entity( fout )) );
    }
...
static bool dd_du( fin, fout )
    effect fin, fout; {
    return ( action_write_p( effect_action( fin )) && ( ( get_bool_property( "CHAINS_DATAFLOW_DEPENDENCE_ONLY" ) ) ? action_read_p( effect_action( fout )) : TRUE ) );
}
        ...
        FOREACH(EFFECT, fout, effect_outs)
        {
            ...
            if ( entity_conflict_p( ein, eout ) && ( *which )( fin, fout ) ) {
                ...
                if ( add_conflict_p ) {
                    ...
                    reference rin = effect_any_reference(fin);
                    reference rout = effect_any_reference(fout);
                    bool remove_this_conflict_p = disambiguate_constant_subscripts ? references_do_not_conflict_p( rin, rout ) : FALSE;
...
graph statement_dependence_graph( statement s ) {
    ...
    disambiguate_constant_subscripts = get_bool_property( "CHAINS_DISAMBIGUATE_CONSTANT_SUBSCRIPTS" );
...
static void set_effects( char *module_name, enum chain_type use ) {
    switch ( use ) {
        ...
        case USE_REGIONS:
            rgch = TRUE;
            iorgch = FALSE;
            set_proper_rw_effects( (statement_effects) db_get_memory_resource( DBR_PROPER_REGIONS, module_name, TRUE ) );
            break;
...
/* Compute chain dependence for a module according different kinds of
 store-like effects.
