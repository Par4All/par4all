...
static bool expand_reference(reference r)
{
...
static bool prepare_and_perform_expansion(loop l)
{
  ...
  if(expression_integer_value(lb, &ilb)
	...
	gen_recurse(bs, reference_domain, expand_reference, gen_null);
	expanded_variable = entity_undefined;
      }
    }, loop_locals(l));

    /* Remove the expanded variables from the local variable list */
