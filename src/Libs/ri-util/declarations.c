      ...
      if ( entity_undefined_p(param) ) {
        ...
        if ( prettyprint_is_fortran )
          ...
          pc
              = gen_nconc( pc,
                           c_words_entity( t,
                                           CHAIN_SWORD(NIL,entity_local_name(param)),
                                           pdl ) );
        }
      }
    }
...
sentence sentence_head( entity e, list pdl ) {
  ...
  switch ( type_tag(tr) ) {
    ...
    case is_type_variable: {
      list pdl = NIL;
      pc
          = gen_nconc( pc, words_basic( variable_basic(type_variable(tr)), pdl ) );
  ...
  if ( !ENDP(args) ) {
    ...
    pc = CHAIN_SWORD(pc, ")");
  } else if ( type_variable_p(tr) || ( !prettyprint_is_fortran
...
static bool empty_static_area_p( entity e ) {
  if ( !static_area_p( e ) )
    return FALSE;
  ...
  if ( !ENDP(area_layout(type_area(te))) ) {
    ...
    if ( !ENDP(entities) ) {
      ...
      return make_sentence( is_sentence_formatted,
                            strdup( concatenate( "!! empty common ",
                                                 entity_local_name( e ),
                                                 " in module ",
...
static text text_area_included( entity common /* the common the declaration of which are of interest */,
  ...
  if ( file_exists_p( file ) ) {
    ...
    string nofile = strdup( concatenate( file,
                                         ".sorry_common_not_homogeneous",
                                         NULL ) );
...
static string default_common_hook( entity __attribute__ ((unused)) module,
                                   entity common ) {
  return strdup( concatenate( "common to include: ",
...
static text text_equivalence_class( list /* of entities */l_equiv ) {
  ...
  while ( !ENDP(l2) ) {
    ...
    if ( offset1 == offset2 ) {
      ...
      if ( first )
        lw = CHAIN_SWORD(lw, "EQUIVALENCE"), first = FALSE;
      else
    ...
    /* Else, we first check that there is an overlap */
       ...
       */
        ...
        int size_elt_1 =
            SizeOfElements( variable_basic(type_variable(entity_type(ent1))) );
 ...
 * comment  :
 */
static text text_equivalences( entity __attribute__ ((unused)) module /* the module dealt with */,
...
static sentence sentence_data_statement( statement is, list pdl ) {
  unformatted u = make_unformatted( NULL,
  ...
  for ( ; !ENDP(al); POP(al) ) {
    ...
    if ( strcmp( module_local_name( call_function(vc) ), REPEAT_VALUE_NAME )
        == 0 ) {
      ...
      if ( !( integer_constant_expression_p( rfe ) && expression_to_int( rfe )
          == 1 ) ) {
...
static text text_entity_declaration( entity module,
  ...
  FOREACH(ENTITY, e,ldecl) {
    ...
    value v = entity_initial(e);
    bool use_p = entity_f95use_p(e);
    bool param = func && value_symbolic_p(v);
    bool external = /* subroutines won't be declared */
    ( func && ( value_code_p(v) || value_unknown_p(v) /* not parsed callee */)
      ...
      switch ( basic_tag(b) ) {
        ...
        case is_basic_float:
          ...
          switch ( basic_float(b) ) {
            case 4:
              pf4
                  = CHAIN_SWORD(pf4, pf4==NIL ? "REAL*4 " : (space_p? ", " : ","));
              pf4 = gen_nconc( pf4, words_declaration( e, pp_dim, pdl ) );
              break;
            case 8:
            default:
              pf8
                  = CHAIN_SWORD(pf8, pf8==NIL ? "REAL*8 " : (space_p? ", " : ","));
        ...
        case is_basic_complex:
          ...
          switch ( basic_complex(b) ) {
            case 8:
              pc8
                  = CHAIN_SWORD(pc8, pc8==NIL ? "COMPLEX*8 " : (space_p? ", " : ","));
              pc8 = gen_nconc( pc8, words_declaration( e, pp_dim, pdl ) );
              break;
            case 16:
            default:
              pc16
                  = CHAIN_SWORD(pc16, pc16==NIL ? "COMPLEX*16 " : (space_p? ", " : ","));
        ...
        case is_basic_string: {
          ...
          if ( value_constant_p(v) && constant_int_p(value_constant(v)) ) {
            ...
            if ( i == 1 ) {
              ps
                  = CHAIN_SWORD(ps, ps==NIL ? "CHARACTER " : (space_p? ", " : ","));
...
text text_declaration( entity module ) {
  /* Assume Fortran only! */
  list pdl = NIL;
  text t = text_entity_declaration( module,
...
list generic_c_words_entity( type t,
  ...
  return generic_c_words_simplified_entity( t,
                                            name,
                                            is_safe,
...
list generic_c_words_simplified_entity( type t,
  ...
  if ( type_functional_p(t) ) {
    ...
    if ( ( gen_length( name ) > 1 ) || ( ( gen_length( name ) == 1 )
        && ( strcmp( STRING(CAR(name)), "*" ) == 0 ) ) ) {
        ...
        pc
            = gen_nconc( pc,
                         generic_c_words_simplified_entity( t1,
                                                            string_undefined_p(pn) ? NIL
                                                                                   : CONS(STRING, strdup(pn), NIL),
                                                            is_safe,
                                                            FALSE,
                                                            TRUE,
                                                            in_type_declaration,
    ...
    pc = CHAIN_SWORD(pc,")");
    return generic_c_words_simplified_entity( t2,
                                              pc,
                                              is_safe,
                                              FALSE,
                                              is_first,
  ...
  if ( pointer_type_p( t ) ) {
    ...
    pc = gen_nconc( pc, name );
    return generic_c_words_simplified_entity( t1,
                                              pc,
                                              is_safe,
                                              FALSE,
                                              is_first,
  ...
  if ( ( is_first || in_type_declaration ) && type_variable_p(t)
      && variable_qualifiers(type_variable(t)) != NIL )
    pc = words_qualifier( variable_qualifiers(type_variable(t)) );
  else if ( ( is_first || in_type_declaration ) && type_void_p(t)
      && type_void(t) != NIL )
    ...
    if ( is_first ) {
      ...
      if ( ( strstr( n, ENUM_PREFIX DUMMY_ENUM_PREFIX ) == NULL )
        ...
        if ( !gen_in_list_p( (void *) ent, pdl ) ) {
          ...
          epc = generic_c_words_simplified_entity( t1,
  ...
  if ( type_struct_p(t) ) {
    ...
    for ( cl = l; !ENDP(cl); POP(cl) ) {
      ...
      pc = gen_nconc( pc,
                      c_words_entity( tsm,
                                      CHAIN_SWORD(NIL,entity_user_name(sm)),
                                      pdl ) );
      if ( ENDP(CDR(cl)) )
        pc = CHAIN_SWORD(pc,";");
      else
  ...
  if ( type_union_p(t) ) {
    ...
    for ( cl = l; !ENDP(cl); POP(cl) ) {
      ...
      pc = gen_nconc( pc,
                      c_words_entity( tu,
                                      CHAIN_SWORD(NIL,entity_user_name(eu)),
                                      pdl ) );
      if ( ENDP(CDR(cl)) )
        pc = CHAIN_SWORD(pc,";");
      else
...
list c_words_simplified_entity( type t,
                                ...
                                bool in_type_declaration,
                                list pdl ) {
  list pc = generic_c_words_simplified_entity( t,
                                               name,
                                               FALSE,
                                               FALSE,
                                               is_first,
...
text c_text_entities( entity module, list ldecl, int margin, list pdl ) {
  ...
  FOREACH(ENTITY, e, ldecl) {
    ...
    if ( !type_area_p(t) && !type_statement_p(t) && !type_unknown_p(t)
        && !storage_formal_p(entity_storage(e)) && !implicit_c_variable_p( e ) ) {
...
static list words_variable_or_function( entity module,
  ...
  pc = gen_nconc( pc, c_words_simplified_entity( t,
                                                 CHAIN_SWORD(NIL,name),
                                                 is_first,
                                                 in_type_declaration,
                                                 pdl ) );
  ...
  if ( !entity_undefined_p(module) && !extern_entity_p( module, e )
    ...
    if ( value_expression_p(val) ) {
      ...
      if ( brace_expression_p( exp ) )
        ...
        /* */
        pc = gen_nconc( pc, words_subexpression( exp,
      ...
      if ( type_variable_p(t) ) {
        ...
        if ( !ENDP(il) ) {
          ...
          expression exp = instruction_expression(statement_instruction(is));
          pc = CHAIN_SWORD(pc," = ");
...
static list filtered_declaration_list( list del ) {
  ...
  if ( gen_length( del ) >= 2 ) {
    ...
    if ( entity_struct_p( e_previous ) || entity_union_p( e_previous )
        || entity_enum_p( e_previous ) ) {
      FOREACH(ENTITY, e, CDR(del))
      {
        if ( entity_struct_p( e ) || entity_union_p( e ) || entity_enum_p( e ) ) {
          POP(el);
        } else {
          break;
        }
...
text c_text_related_entities( entity module,
  ...
  if ( !entity_undefined_p(module) && ( explicit_extern_entity_p( module,
                                                                  e_last )
  ...
  switch ( type_tag(t1) ) {
    ...
    case is_type_unknown: {
      ...
      pc = words_variable_or_function( module,
                                       e1,
                                       TRUE,
                                       pc,
                                       in_type_declaration,
...
void print_C_common_layout( FILE * fd, entity c, bool debug_p ) {
  ...
  (void) fprintf( fd,
                  ...
                  entity_name(c),
                  area_size(type_area(entity_type(c))) );
...
void fprint_any_environment( FILE * fd, entity m, bool is_fortran ) {
  ...
  (void) fprintf( fd, "%s\n", ENDP(decls) ? "* empty declaration list *\n\n"
                                          : "Variable list:\n\n" );
  ...
  if ( !is_fortran ) {
    ...
    (void) fprintf( fd,
                    "%s\n",
                    ENDP(edecls) ? "* empty external declaration list *\n\n"
