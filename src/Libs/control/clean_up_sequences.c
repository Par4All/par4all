		...
		FOREACH(ENTITY, decl, l_internal_proper_decls)
		  {
			...
			// generated by PIPS pĥases without scope
			pips_debug(6, "non consistant variable scope: keep original name\n");
		      }
		    else if (decl_without_module_and_scope[-1]=='0' && decl_without_module_and_scope[-2]== MODULE_SEP_STRING[0])
		      {
			pips_debug(6, "Entity already at the uppermost scope level: keep it\n");
		      }
		    else
		      {
