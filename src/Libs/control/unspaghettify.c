...
/* Remove a useless continue in a sequence of control since it is
   ...
   controlizer... */
static void
remove_useless_continue_or_empty_code_in_unstructured(statement s,
                                                      unstructured u)
{
   list blocs = NIL;
...
fuse_sequences_in_unstructured(unstructured u)
{
   ...
   HASH_MAP(the_original_control,
            its_adress_now,
            {
               ...
               pips_assert("control inconsistants...", gen_consistent_p(a_control_to_fuse));
               
               its_successor = CONTROL(CAR(control_successors(a_control_to_fuse)));
               pips_assert("control inconsistants...", gen_consistent_p(its_successor));
               the_third_successor = CONTROL(CAR(control_successors(its_successor)));
               pips_assert("control inconsistants...", gen_consistent_p(the_third_successor));
...
static bool
try_to_structure_the_unstructured(statement s, instruction i, unstructured u)
{
   control end_of_first_sequence, begin_of_last_sequence, c;
   ...
   if (begin_statement_list != NIL || end_statement_list != NIL) {
      if (end_of_first_sequence == exit_node) {
         ...
         list list_of_the_new_statements = CONS(STATEMENT,
...
unspaghettify_rewrite_unstructured(statement s, instruction i, unstructured u)
{
   ...
   if (!try_to_structure_the_unstructured(s, i, u)) {
      ...
      remove_useless_continue_or_empty_code_in_unstructured(s, u);
