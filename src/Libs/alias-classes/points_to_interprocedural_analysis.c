...
/* Foreach real argument r and its corresponding formal one f, create
 * the assignment "f = r;" then compute points-to set s generated by the
 * assignment. The result is the union of pt_caller and s
 */
...
set points_to_binding(list args, set in, set pt_binded)
{
   ...
   *
   * The same issue should/could arise when arrays are passed as
   * argument. See effectsWithPointsTo/call22.c.
   */
  set_assign(bm1, bm);
