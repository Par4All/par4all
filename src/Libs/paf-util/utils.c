...
vertex v;
{
        ...
        debug(9, "in_dfg_vertex_list", "doing \n");
...
vertex v;
{
        ...
        debug(9, "in_dg_vertex_list", "doing \n");
 ...
 * exist yet.
 */
expression make_id_expression(s)
string s;
{
 entity new_ent;
...
hash_table STS;
{
  ...
  for(; dfg_edge_l != NIL; dfg_edge_l = CDR(dfg_edge_l)) {
    ...
    dfg_vertex_label_exec_domain(dvl) = make_predicate((char *) new_sc);
   }
}
 ...
 */
Psysteme make_expression_equalities(le)
list le;
{
 Psysteme new_sc;
 list l;
...
static list find_el_with_num(stmt)
int stmt;
{
 hash_table scm = get_current_stco_map();

 static_control stct = (static_control) hash_get(scm, (char *) stmt);
 ...
 * "v".
 */
successor first_succ_of_vertex(v)
vertex v;
{
...
/*============================================================================*/
/* Ppolynome pu_expression_to_polynome(expression exp): translates "exp" into a
 ...
 */
Ppolynome pu_expression_to_polynome(exp)
expression exp;
{
 Ppolynome pp_new; /* This the resulting polynome */
 ...
 switch(syntax_tag(sy))
   {
    case is_syntax_reference:
	  ...
	  user_error("pu_expression_to_polynome", "Only scalars in the polynome");
	...
	*/
       ...
       else
	 {
	  ...
	  if(! ENTITY_FOUR_OPERATION_P(op_ent))
	     user_error("pu_expression_to_polynome",
			"Call to %s, operation must be : +, -, * or /",
			entity_local_name(op_ent));
	  ...
	  if( (cl != 2) && (cl != 1) )
	     user_error("pu_expression_to_polynome", "%s call with %d argument(s)",
	  ...
	  if(cl == 2)
	    ...
	    {
             pp_new = pu_expression_to_polynome(arg1);
	     polynome_add(&pp_new, pu_expression_to_polynome(arg2));
	    }
	  else if(ENTITY_MINUS_P(op_ent)) /* -arg2 + arg1 */
	    {
             pp_new = pu_expression_to_polynome(arg2);
	     polynome_negate(&pp_new);
	     polynome_add(&pp_new, pu_expression_to_polynome(arg1));
	    }
	  else if(ENTITY_MULTIPLY_P(op_ent)) /* arg1 * arg2 */
	     pp_new = polynome_mult(pu_expression_to_polynome(arg1),
				    pu_expression_to_polynome(arg2));
	  else if(ENTITY_DIVIDE_P(op_ent)) /* arg1 / arg2 */
	     pp_new = polynome_div(pu_expression_to_polynome(arg1),
				    pu_expression_to_polynome(arg2));
    ...
    default :
      {
       if (syntax_tag(sy) == is_syntax_range)
         debug(6, "pu_expression_to_polynome", " Expression RANGE\n");
       pips_error("pu_expression_to_polynome", "Bad syntax tag");
      }
   }
 return(pp_new);
}
 ...
 * is of degree one (1).
 */
Pvecteur polynome_to_vecteur(pp)
Ppolynome pp;
{
 ...
 */
boolean is_entity_in_list_p(e, l)
entity e;
list l;
{
 ...
 */
boolean single_var_vecteur_p(pv)
Pvecteur pv;
{
 ...
 * Ppolynome.
 */
Ppolynome old_vecteur_to_polynome(vec)
Pvecteur vec;
{
 ...
 * Ppolynome.
 */
Ppolynome vecteur_to_polynome(pv)
Pvecteur pv;
{
  Ppolynome pp;
 ...
 *  I tried the one above, but I could not go thru the validatation...
 *
 *  FC 02/01/94
 */
...
boolean (*compare_obj)();
{
  ...
  chunk * crt_obj, * prev_obj;

  debug(9, "general_merge_sort", "Debut\n");
  ...
  debug(9, "general_merge_sort", "Fin\n");
 ...
 * rational calculus.
 */
expression rational_op_exp(op_name, exp1, exp2)
char *op_name;
expression exp1, exp2;
{
  ...
  debug(5, "rational_op_exp", "begin OP EXP : %s  %s  %s\n",
  ...
  if( expression_constant_p(exp1) && expression_constant_p(exp2) ) {
    ...
    debug(6, "rational_op_exp", "Constant expressions\n");
    ...
    if((normalized_tag(nor1) == is_normalized_linear) &&
      ...
      debug(6, "rational_op_exp", "Linear operation\n");
  ...
  debug(5, "rational_op_exp", "end   OP EXP : %s\n",
...
Ppolynome ppsubst;
{
  ...
  if(POLYNOME_NUL_P(ppsubst)) {
    ...
    for(ppp = newpp; ppp != NULL; ppp = ppp->succ) {
      ...
      if(pv != NULL)
        pips_error("prototype_var_subst", "Vecteur should contains 2 var\n");
 ...
 */
Pvecteur prototype_factorize(pp, var)
Ppolynome pp;
Variable var;
{
    Pvecteur pv = NULL;
 ...
 * equation).
 * */
Psysteme find_implicit_equation(ps)
Psysteme ps;
{
  Pcontrainte ineg, eg;
...
/* ======================================================================== */
void set_current_stco_map(scm)
statement_mapping scm;
{
  pips_assert("set_current_stco_map", current_stco_map ==
...
/* ======================================================================== */
static_control get_stco_from_current_map(s)
statement s;
{
