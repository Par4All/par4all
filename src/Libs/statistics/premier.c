...
int  floor ;
{
  if (floor==1)
   return 
     myfloor(
	     vect_coeff(TCST,pv2) -vect_coeff(TCST,pv1),
...
{ 
  ...
  pips_assert("normalized are linear", 
	      normalized_linear_p(norm1) &&
  ...
  if (instruction_sequence_p(ins))
	...
	if   (instruction_call_p(ins1 )) {
	  ...
	  if(strcmp(entity_name(  call_function(ca)),"TOP-LEVEL:CONTINUE" )!=0){
	    ...
	    else {  CDR(lis1par)=CONS(STATEMENT,s,NIL);lis1par=CDR(lis1par); };
	  };
	}
	else 
	  {
	...
	if (instruction_call_p(ins1 )) {
	  call ca= instruction_call(ins1 ) ;
	  if(  strcmp(  entity_name(  call_function(ca)),"TOP-LEVEL:CONTINUE" )!=0){
	if (i==1)  { lis2=CONS(STATEMENT,s,NIL); lis2par=lis2;}
	else  { CDR(lis2par)=CONS(STATEMENT,s,NIL); lis2par=CDR(lis2par);};
	  };
	}
    else {
      if (i==1)  { lis2=CONS(STATEMENT,s,NIL); lis2par=lis2;}
	...
	if (gen_length(lis1)!=0)
	  ...
	  t1= make_test(eq_expression (copyindex,copylower),s1, make_block_statement(NIL));
	...
	if (gen_length(lis2)!=0)  
	  { expression copyindex, copyupper;
	    ...
	    t2= make_test(eq_expression (copyindex,copyupper) ,s2, make_block_statement(NIL));
...
/* static void clean_stmt(statement s)
   {
...
void sys_matrix_index(Psysteme s, Pbase b, Pmatrix A)
{
  printf(" entrer \n");
  sys_matrice_index(s, b, A->coefficients, 
		    A->number_of_lines, A->number_of_lines);
}  */
     ...
     statement s;
{
  ...
  for (e = vertice; e != NULL; e = e->succ) {
    ...
    pv1=e->vecteur;
     ...
     printf(" %d   %lld %lld \n",i,vect_coeff(indice1,pv1),e->denominateur  );
     i++;
  };
  ...
  for(i=0;i<=2*nbr_vertice-2;i++){ 
	...
	if (val2 < 0){
	  ...
	  if (  eval2(constante,val1,val2, sommetg[indice])==minjp ){
	    ...
	    if(value_direct_multiply( value_direct_multiply(val1,sommetg[indice])+ constante ,-val2p)
	       > value_direct_multiply( value_direct_multiply (val1p,sommetg[indice])+ constantep ,-val2))
	      cinf=peq;
	    else
	      if(value_direct_multiply(value_direct_multiply(val1,sommetg[indice])+ constante , -val2p)
		 ==value_direct_multiply( value_direct_multiply (val1p,sommetg[indice])+ constantep,-val2))
  ...
  i=-1;
  lisp=lis; 
  /*   MAP(STATEMENT,s,{i++;if((i!=isoler)&&(i%2==0)) {
    if(i==0) {
...
bool guard_elimination(string module)
{
  ...
  s1=ordering_to_statement(ordering);
  MAP(STATEMENT,s,{if (statement_ordering(s)==ordering){ s1=unimodular(s1);s1= free_guards(s1); 
