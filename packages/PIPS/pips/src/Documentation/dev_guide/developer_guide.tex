...
In order to help preventing this situation anyway, PIPS is checked
every minute and possibly fully recompiled.
...
state right now, but the new code should be documented.
...
\emph{You should not do memory leaks}. First because it is wicked and secondly
because it often hides (or exhibits? \smiley) nasty bugs that will be
discovered by some other people or your children may be 20 years later and
who will want to kill you during your retirement\footnote{Ronan
  \textsc{Keryell} is just thinking to the \texttt{controlizer} right
...
be an svn working copy of the validation repository. Moreover, there
must be a \texttt{Makefile} within each directory, which defines a
\texttt{PARALLEL\_VALIDATION} macro if the validation can run in
parallel. The typical (very short) \texttt{Makefile} is:
\begin{verbatim}
...
For the \textbf{intra}-directory parallel validation, all validation
scripts must be \textbf{without any side effects}, that is temporary
files must be fully isolated from those generated by other scripts.
%%
To run an intra-directory parallel validation:
\begin{verbatim}
  sh> cd validation/HardwareAccelerator
  sh> make -j 8 validate-test
\end{verbatim}
Then local file \texttt{failed} contains a summary of results.
...
For the \textbf{inter}-directory parallel validation, on defaults or
only some directories by defining the \texttt{TARGET} make macro:
\begin{verbatim}
  sh> cd validation
...
\item[failed] the test failed (abort, segfault\ldots).
...
To revert all changes, and cleanup everything do at the root of the
validation:
\begin{verbatim}
