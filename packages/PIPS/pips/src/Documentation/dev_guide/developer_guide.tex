...
\begin{description}
\item[CHECK] on every commit (down to the few minute scale) compilation and
  validation based on the old but fast \texttt{nlpmake} makefiles
  on Debian Lenny (compiegne).
  ...
  The whole compilation and validation runs in about 7 minutes.

\item[AUTOC] on every hour, compilation and validation with the \texttt{autoconf}
  infrastructure, including python and fortran95, on Debian Lenny (compiegne).

  This compilation and validation requires about 13 minutes.
...
\textbf{Note that these scripts are more or less obsolete.
...
validation files within a directory are run in parallel).
%%
