...
live_paths     > MODULE.live_in_paths
        ...
        < PROGRAM.entities
        < MODULE.code
        < CALLEES.live_out_summary_paths
\end{PipsMake}
...
Property \PipsPropRef{MEMORY_IN_OUT_REGIONS_ONLY}'s default value is
set to {\tt TRUE} to avoid computing IN and OUT regions on non-memory
effects, even if \PipsPropRef{MEMORY_EFFECTS_ONLY} is set to {\tt
  FALSE}.

\begin{PipsProp}{MEMORY_IN_OUT_REGIONS_ONLY}
...
\begin{PipsPass}{simplify_control_directly}
  ...
  yet another time. The property \PipsPropRef{SIMPLIFY_CONTROL_DIRECTLY_PRIVATE_LOOP_INDICES}
...
are declared into this new compilation unit\marginpar{Serge: Consistency
  betweeen properties? Name conflicts?}.
...
Property \PipsPropRef{SCALARIZATION_FORCE_OUT} forces the generation of a copy-out statement
when it is useless according to the OUT region, for instance when dealing
...
Property \PipsPropRef{SCALARIZATION_STRICT_MEMORY_ACCESSES} is used to
control the insertion of new memory accesses. A memory access may be
moved out of a loop, which is the best case for performance, but it
then is no longer control dependent on the loop bounds. A new access
is thus added if the compiler cannot prove that the loop is always
entered. However, the burden of the proof may be too much, if only
because nothing can be proven when the loop is sometimes entered and
sometimes not. Also, the memory access may be always perfectly
legal. The default value is FALSE, going for performance before
safety. This property is pretty hard to understand, as well as its
impact. See sequence06, 07 and 08 in the validaiton suite of the
scalarization pass.
\begin{PipsProp}{SCALARIZATION_STRICT_MEMORY_ACCESSES}
SCALARIZATION_STRICT_MEMORY_ACCESSES FALSE
\end{PipsProp}
...
\begin{PipsPass}{scop_outliner}
  This pass is used for outlining the static control parts (SCoP)
  computed by the previous pass, pocc prettyprinted.  Instrumented
  code can be an input for any PoCC compiler.
\end{PipsPass}
...
For the outlinig of the control static parts, function prefix names to
be used during the generation:
\begin{PipsProp}{SCOP_PREFIX}
SCOP_PREFIX "SCoP"
\end{PipsProp}

To generalize the outlining for other purposes, the two delimiter
pragma used can be redefined using the next two
properties:
\begin{PipsProp}{PRAGMA_BEGIN_OUTLINING}
PRAGMA_BEGIN_OUTLINING "scop"
\end{PipsProp}
\begin{PipsProp}{PRAGMA_END_OUTLINING}
PRAGMA_END_OUTLINING "endscop"
\end{PipsProp}
which maybe redundant with property
\PipsPropRef{EXTERNALIZED_CODE_PRAGMA_BEGIN} of libraries
\verb/phrase/ and \verb/safescale/. But both of these libraries are
currently not maintained and not tested.
