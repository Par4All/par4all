...
\title{PIPS: Mechanism for the automated management of the coherence and the 
successive manifestations of phases(pipsmake)}
...
The interprocedural and interoperative nature of PIPS make necessary 
the management of partial information, linked to a particular module 
and phase, which may be reused 
when different calculations of successive sessions occur. 

The successive manifestations of the calculation of these informations and the coherence tests 
could have been spread out in each of the phases and analysis of PIPS.
It seamed preferable to centralize in one library the management 
of dependency between phases and the maintenance of the coherence between 
different information.

The library {\tt pipsmake} offers two principal points of entry:
{\tt make} and {\tt apply}. The first permits the one to maintain the particular information
without having to worr about the calculation of all the
different information which is necessairy for its calculation. The second permits
the evaluation of a particular rule.

When several rules allow the calculation of a particular objet, 
an activation mechanism allows the definition of that which needs to be used. 
This is indispensible to be able to treat the recursive calls 
produced by the succesive manifestation of rules without having to 
...
in a file of name {\tt pipsmake.rc}. The library 
{\tt pipsmake} contains the modules which permit the readiong of such a file 
...
is activated. When several rules are available to produce 
the same resource, this resource must be the only one produced
by this rule. In this manner it is coherent to dynamically change 
active rules.

This is automatiocally rendered possible by certain interactive pips interfaces,
...
Kennedy}; 
\item[dgwolfe] dependancy graph with the {\em vecteurs de direction de
Wolfe};
\end{description}
}

\domain{Owner = \{ program , module , main , callees , callers , all ,  select , compilation_unit \}}
{The domain {\tt owner} permits the precise description of the dependency rule
...
The real resources correspond to a particular set of data 
produced by a module or a particular program by a particular
phase. The virtual resources take their value among these 
real resources, but these derivation rules of pipsmake are still generic and therefore still defined in terms of
de virtual resources. 
