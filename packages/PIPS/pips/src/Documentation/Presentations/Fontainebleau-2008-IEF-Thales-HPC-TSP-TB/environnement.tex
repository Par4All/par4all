...
\begin{trans}{Pourquoi un établi ?}
  ...
  \item Même usage d'analyses et transformations de programme avancées
    ...
    \item Restructuration \& ingénierie à rebour
    \end{itemizet}
  \end{itemizec}
...
\begin{trans}{Phases d'analyse}
  ...
  \item Graphe de flot de données, planification, base de temps et
    répartition
  \item Détection de réductions
  \item Complexités symboliques
  \item Fortran \texttt{lint}
  \item Ajout d'autres domaines d'interprétation abstraite : projet APRON
  \end{itemizer}
\end{trans}
...
\begin{trans}{Plan du rajout de phase}
  ...
  \item Gestionnaire de consistance interprocédural PIPSmake
...
\input{codes/dead_statement_filter_fc}
  ...
  \item Itérateurs génériques optimisés
  \item méthodes multi-languages (C \& Common-Lisp pour l'instant)
  \item Support pour la persistance (PIPSdbm)
  \item Construction possible de passes indépendantes (sans PIPSmake) :
    EOLE
  \end{itemizer}
\end{trans}
...
\begin{trans}{Programmation littérale}
  ...
  versions de développement (branches), production (tronc) et distribution
  (tag, release)
\end{trans}
...
\begin{trans}{Le recette finale}
  ...
  \item Compiler et tester avec Makefile inspiré d'autres
    bibliothèques/phases
