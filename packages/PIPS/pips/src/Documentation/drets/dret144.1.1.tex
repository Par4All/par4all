...
\newcommand{\titre}{PROJET PIPS \\
		Manuel Utilisateur du Paralléliseur Batch (Version 1.1)}
\newcommand{\auteur}{
        	François IRIGOIN \\
        	Pierre JOUVELOT\\
\vspace{0.5cm}
...
\newcommand{\docdate}{15 Décembre 1990}
\newcommand{\numero}{E144}
...
installé
  ...
  \item mettre à jour la variable shell PIPSDIR dans les fichiers {\tt pipsrc.sh} et {\tt
pipsrc.csh} en fonction du répertoire qui a été choisi; il est préférable
de lui donner un nom absolu;

  \item exécuter celui de ces deux fichiers qui correspond à l'analyseur
...
La manipulation directe du fichier \verb+pipsmake.rc+ est aussi à éviter.
...
Les alias sont implémentés dans le shell-script de Build (présent
