/*
  ...
  $Id: phppips.c 20843 2011-12-31 08:40:08Z coelho $
...
static void phpips_log_handler(const char *fmt, va_list args)
{
...
char * phpips_last_error = NULL;
static void phpips_error_handler(const char * calling_function_name,
                   const char * a_message_format,
                   va_list *some_arguments)
{
	// Save pre-existing error message
	char *old_error = phpips_last_error;

    char * tmp;
    vasprintf(&tmp,a_message_format,*some_arguments);
    asprintf(&phpips_last_error,"in %s: %s",calling_function_name,tmp);
    free(tmp);

    // If we already had a message before, we stack it over the new one
    if(old_error) {
    	char *tmp = phpips_last_error;
        asprintf(&phpips_last_error,"%s%s",old_error,tmp);
    	free(old_error);
    	free(tmp);
    }
...
void atinit()
{
    ...
    pips_log_handler = phpips_log_handler;
    pips_error_handler = phpips_error_handler;
...
void apply(char * phasename, char * target)
{
    if (!safe_apply(phasename,target)) {
        if(!phpips_last_error) asprintf(&phpips_last_error,"phase %s on module %s failed" , phasename, target);
        THROW(user_exception_error);
    }
}
