 ...
 * effect on pt_cur or allocation of a new set?)
 *
 * If lhs is not a pointer, return pt_cur.
 *
 * Else:
 *
 *  if lhs can be converted into a cell c
 *
 *     if rhs is a user call and/or contains a user call, since we are
 *     using an intraprocedural analysis, generate (c, anyhwere, may);
 *     remove all points-to starting with c;
 *
 *     else if rhs is an intrinsic call to &,
 *
 *        if the argument of & is a reference that can be converted
 *        into a cell c', generate (c, c', EXACT)
 *
 *        else, generate (c, anywhere, MAY)
 *
 *     else if rhs a reference that can be converted into a cell c'
 *
 *        if c' points to c1, c2, c3,.. then add (c, c1, MAY/EXACT),
 *        (c, c2, MAY),...
 *
 *     else
 *
 ...
 */
set points_to_general_assignment(statement s,
				 expression lhs,
				 expression rhs,
				 set pt_cur)
{
  ...
  if(ENTITY_ASSIGN_P(e)){
    ...
    if(set_undefined_p(pt_out)){
      ...
      */
      ...
      pt_out = points_to_general_assignment(s, lhs, rhs, pt_cur);
    }
  }
