...
static set control_to_points_to(control c, set in, __attribute__ ((__unused__))bool store)
{
  ...
  statement s = control_statement(c);
      out =  statement_to_points_to(s, in);
...
*/
static set cyclic_graph_to_points_to(set ctrls,
			      set in,
 ...
 */
set new_points_to_unstructured(unstructured uns, set pt_in, bool __attribute__ ((__unused__))store){
...
/* #endif */
  ...
  while(!set_empty_p(rtbp)) {
    ...
    SET_FOREACH(control, n , rtbp) {
      ...
      SET_FOREACH(control, p , inter){
        out = control_to_points_to(p, out,true);
      }
      out = control_to_points_to(n, out, true);
  ...
  out = cyclic_graph_to_points_to(UnProcessed, out, store);
  tmp = set_difference(tmp, Nodes, Reachable);
  SET_FOREACH(control, cc, tmp) {
    pt_in_n = set_clear(pt_in_n);
    out = statement_to_points_to(control_statement(cc), out);
  }
  ...
  out = statement_to_points_to(exit_s, out); 
  return out;
}
