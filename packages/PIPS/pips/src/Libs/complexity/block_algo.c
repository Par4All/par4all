...
list effects_list;
{
	...
	for ( var in (var_comp and var_must) or (var_comp and block_var_must) ) {
	    ...
	    /* else perform nothing, which means that this variable can be postponed 
	     to be evaluated*/
	}
    ...
    final_var_comp = var_in_comp(ctemp);
    /* Get the variable from quasi-final complexity result */

    for ( final_var in (final_var_comp and block_var_must) ) {
	/* this is the worstcase */
	/* Use any UU to substitute that final_var */
    }
