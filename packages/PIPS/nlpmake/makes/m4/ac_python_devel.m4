...
$ac_distutils_result])
	...
	if test -z "$PYTHON_LDFLAGS"; then
		...
		PYTHON_LDFLAGS=`$PYTHON -c "from distutils.sysconfig import *; \
            s='-L' + get_python_lib(0,1)+'/config'+' -lpython' ;\
			print(s);"`$py_version
	fi
  ...
  Another possible issue is the lack of some extra developpment headers.
  Libraries required by python are $PYTHON_EXTRA_LIBS
  ============================================================================
   ERROR!
