...
void fdtd__d2(int _u_Ets_n0,int _u_Ets_n1,double _u_Ets[_u_Ets_n0][_u_Ets_n1])
{
  ...
  scilab_rt_zeros_i0i0_d2(_u_Nt,3,_u_Ets_n0,_u_Ets_n1,_u_Ets);
  _u_Ex[0][1][1] = 1;
  _u_Ey[1][0][1] = 2;
  _u_Ez[1][1][0] = 3;
  ...
  for (int _u_n=1; _u_n<=_u_Nt; _u_n++) {
    ...
    for (int _u_i=1; _u_i<=(_u_Nx+1); _u_i++) {
      for (int _u_j=1; _u_j<=_u_Ny; _u_j++) {
        for (int _u_k=1; _u_k<=_u_Nz; _u_k++) {
          double _tmpxx2 = (((_u_Ey[_u_i-1][_u_j-1][(_u_k+1)-1]-_u_Ey[_u_i-1][_u_j-1][_u_k-1])*_u_Cz)-((_u_Ez[_u_i-1][(_u_j+1)-1][_u_k-1]-_u_Ez[_u_i-1][_u_j-1][_u_k-1])*_u_Cy));
          _u_Hxt[_u_i-1][_u_j-1][_u_k-1] = _tmpxx2;
        }
      }
    }
    double _tmpxx3 = (_u_Dt / _u_mu0);
    double _tmpxx4[26][20][15];
    scilab_rt_mul_d0d3_d3(_tmpxx3,26,20,15,_u_Hxt,26,20,15,_tmpxx4);
    double _tmpxx5[26][20][15];
    scilab_rt_add_d3d3_d3(26,20,15,_u_Hx,26,20,15,_tmpxx4,26,20,15,_tmpxx5);
    
    scilab_rt_assign_d3_d3(26,20,15,_tmpxx5,26,20,15,_u_Hx);
    /* Hy = Hy + (Dt/mu0) * ( (Ez(2:Nx+1, :, :) - Ez(1:Nx, :, :)) * Cx - (Ex(:, :, 2:Nz+1) - Ex(:, :, 1:Nz)) * Cz ); */
    for (int _u_i=1; _u_i<=_u_Nx; _u_i++) {
      for (int _u_j=1; _u_j<=(_u_Ny+1); _u_j++) {
        for (int _u_k=1; _u_k<=_u_Nz; _u_k++) {
          double _tmpxx6 = (((_u_Ez[(_u_i+1)-1][_u_j-1][_u_k-1]-_u_Ez[_u_i-1][_u_j-1][_u_k-1])*_u_Cx)-((_u_Ex[_u_i-1][_u_j-1][(_u_k+1)-1]-_u_Ex[_u_i-1][_u_j-1][_u_k-1])*_u_Cz));
          _u_Hyt[_u_i-1][_u_j-1][_u_k-1] = _tmpxx6;
        }
      }
    }
    double _tmpxx7 = (_u_Dt / _u_mu0);
    double _tmpxx8[25][21][15];
    scilab_rt_mul_d0d3_d3(_tmpxx7,25,21,15,_u_Hyt,25,21,15,_tmpxx8);
    double _tmpxx9[25][21][15];
    scilab_rt_add_d3d3_d3(25,21,15,_u_Hy,25,21,15,_tmpxx8,25,21,15,_tmpxx9);
    
    scilab_rt_assign_d3_d3(25,21,15,_tmpxx9,25,21,15,_u_Hy);
    /* Hz = Hz + (Dt/mu0) * ( (Ex(:, 2:Ny+1, :) - Ex(:, 1:Ny, :)) * Cy - (Ey(2:Nx+1, :, :) - Ey(1:Nx, :, :)) * Cx ); */
    for (int _u_i=1; _u_i<=_u_Nx; _u_i++) {
      for (int _u_j=1; _u_j<=_u_Ny; _u_j++) {
        for (int _u_k=1; _u_k<=(_u_Nz+1); _u_k++) {
          double _tmpxx10 = (((_u_Ex[_u_i-1][(_u_j+1)-1][_u_k-1]-_u_Ex[_u_i-1][_u_j-1][_u_k-1])*_u_Cy)-((_u_Ey[(_u_i+1)-1][_u_j-1][_u_k-1]-_u_Ey[_u_i-1][_u_j-1][_u_k-1])*_u_Cx));
          _u_Hzt[_u_i-1][_u_j-1][_u_k-1] = _tmpxx10;
        }
      }
    }
    double _tmpxx11 = (_u_Dt / _u_mu0);
    double _tmpxx12[25][20][16];
    scilab_rt_mul_d0d3_d3(_tmpxx11,25,20,16,_u_Hzt,25,20,16,_tmpxx12);
    double _tmpxx13[25][20][16];
    scilab_rt_add_d3d3_d3(25,20,16,_u_Hz,25,20,16,_tmpxx12,25,20,16,_tmpxx13);
    
    scilab_rt_assign_d3_d3(25,20,16,_tmpxx13,25,20,16,_u_Hz);
    ...
    for (int _u_i=1; _u_i<=_u_Nx; _u_i++) {
      for (int _u_j=2; _u_j<=_u_Ny; _u_j++) {
        for (int _u_k=2; _u_k<=_u_Nz; _u_k++) {
          double _tmpxx14 = (((_u_Hz[_u_i-1][_u_j-1][_u_k-1]-_u_Hz[_u_i-1][(_u_j-1)-1][_u_k-1])*_u_Cy)-((_u_Hy[_u_i-1][_u_j-1][_u_k-1]-_u_Hy[_u_i-1][_u_j-1][(_u_k-1)-1])*_u_Cz));
          _u_Ext[_u_i-1][_u_j-1][_u_k-1] = _tmpxx14;
        }
      }
    }
    for (int _u_i=1; _u_i<=_u_Nx; _u_i++) {
      for (int _u_j=2; _u_j<=_u_Ny; _u_j++) {
        for (int _u_k=2; _u_k<=_u_Nz; _u_k++) {
          double _tmpxx15 = (_u_Ex[_u_i-1][_u_j-1][_u_k-1]+((_u_Dt / _u_eps0)*_u_Ext[_u_i-1][_u_j-1][_u_k-1]));
          _u_Ex[_u_i-1][_u_j-1][_u_k-1] = _tmpxx15;
        }
      }
    }
    ...
    for (int _u_i=2; _u_i<=_u_Nx; _u_i++) {
      for (int _u_j=1; _u_j<=_u_Ny; _u_j++) {
        for (int _u_k=2; _u_k<=_u_Nz; _u_k++) {
          double _tmpxx16 = (((_u_Hx[_u_i-1][_u_j-1][_u_k-1]-_u_Hx[_u_i-1][_u_j-1][(_u_k-1)-1])*_u_Cz)-((_u_Hz[_u_i-1][_u_j-1][_u_k-1]-_u_Hz[(_u_i-1)-1][_u_j-1][_u_k-1])*_u_Cx));
          _u_Eyt[_u_i-1][_u_j-1][_u_k-1] = _tmpxx16;
        }
      }
    }
    for (int _u_i=2; _u_i<=_u_Nx; _u_i++) {
      for (int _u_j=1; _u_j<=_u_Ny; _u_j++) {
        for (int _u_k=2; _u_k<=_u_Nz; _u_k++) {
          double _tmpxx17 = (_u_Ey[_u_i-1][_u_j-1][_u_k-1]+((_u_Dt / _u_eps0)*_u_Eyt[_u_i-1][_u_j-1][_u_k-1]));
          _u_Ey[_u_i-1][_u_j-1][_u_k-1] = _tmpxx17;
        }
      }
    }
    ...
    for (int _u_i=2; _u_i<=_u_Nx; _u_i++) {
      for (int _u_j=2; _u_j<=_u_Ny; _u_j++) {
        for (int _u_k=1; _u_k<=_u_Nz; _u_k++) {
          double _tmpxx18 = (((_u_Hy[_u_i-1][_u_j-1][_u_k-1]-_u_Hy[(_u_i-1)-1][_u_j-1][_u_k-1])*_u_Cx)-((_u_Hx[_u_i-1][_u_j-1][_u_k-1]-_u_Hx[_u_i-1][(_u_j-1)-1][_u_k-1])*_u_Cy));
          _u_Ezt[_u_i-1][_u_j-1][_u_k-1] = _tmpxx18;
        }
      }
    }
    for (int _u_i=2; _u_i<=_u_Nx; _u_i++) {
      for (int _u_j=2; _u_j<=_u_Ny; _u_j++) {
        for (int _u_k=1; _u_k<=_u_Nz; _u_k++) {
          double _tmpxx19 = (_u_Ez[_u_i-1][_u_j-1][_u_k-1]+((_u_Dt / _u_eps0)*_u_Ezt[_u_i-1][_u_j-1][_u_k-1]));
          _u_Ez[_u_i-1][_u_j-1][_u_k-1] = _tmpxx19;
        }
      }
    }
