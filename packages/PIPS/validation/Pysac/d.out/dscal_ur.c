...
void dscal_ur(int n, float da, float dx[n])
{
   ...
   int LU_IND00, LU_IND01, LU_IB00, LU_NUB00, i0, i1, m0;
   ...
   for(LU_IND00 = 0; LU_IND00 <= LU_IB00-1; LU_IND00 += 1) {
      //PIPS:SAC generated v4sf vector(s)
      SIMD_LOAD_V4SF(vec20_0, &dx[m0+5*LU_IND00]);
      SIMD_MULPS(vec20_0, vec30_0, vec20_0);
      SIMD_STORE_V4SF(vec20_0, &dx[m0+5*LU_IND00]);
      dx[m0+5*LU_IND00] = pdata0[0]*dx[m0+5*LU_IND00];
      dx[4+m0+5*LU_IND00] = pdata0[0]*dx[4+m0+5*LU_IND00];
   }
   SIMD_LOAD_V4SF(vec50_0, &pdata0[0]);
   for(LU_IND01 = LU_IB00; LU_IND01 <= LU_NUB00-1; LU_IND01 += 4) {
      //PIPS:SAC generated v4sf vector(s)
      SIMD_LOAD_V4SF(vec40_0, &dx[m0+5*LU_IND01]);
      SIMD_MULPS(vec40_0, vec50_0, vec40_0);
      SIMD_STORE_V4SF(vec40_0, &dx[m0+5*LU_IND01]);
      SIMD_LOAD_V4SF(vec60_0, &dx[4+m0+5*LU_IND01]);
      SIMD_MULPS(vec60_0, vec50_0, vec60_0);
      SIMD_STORE_V4SF(vec60_0, &dx[4+m0+5*LU_IND01]);
      SIMD_LOAD_V4SF(vec80_0, &dx[8+m0+5*LU_IND01]);
      SIMD_MULPS(vec80_0, vec50_0, vec80_0);
      SIMD_STORE_V4SF(vec80_0, &dx[8+m0+5*LU_IND01]);
      SIMD_LOAD_V4SF(vec100_0, &dx[12+m0+5*LU_IND01]);
      SIMD_MULPS(vec100_0, vec50_0, vec100_0);
      SIMD_STORE_V4SF(vec100_0, &dx[12+m0+5*LU_IND01]);
      SIMD_LOAD_V4SF(vec120_0, &dx[16+m0+5*LU_IND01]);
      SIMD_MULPS(vec120_0, vec50_0, vec120_0);
      SIMD_STORE_V4SF(vec120_0, &dx[16+m0+5*LU_IND01]);
   }
   ;
}
int main(int argc, char *argv[])
