...
// S_L = {(x, y) | (x, y) in S and y < 2x-2}
// S_U = {(x, y) | (x, y) in S and y >= 2x-2}
// 
...
#define S_L {assert(G1 && y < 2 * x - 2);}
#define S_U {assert(G2 && y >= 2 * x - 2);}

#define T1_LL T1
#define T2_LL {trans(G2 && y < 2 * x - 3, U2);}
#define T1_UU {trans(G1 && y >= 2 * x, U1);}
#define T2_UU T2
#define T1_UL {trans(G1 && y < 2 * x, U1);}
#define T2_LU {trans(G2 && y >= 2 * x - 3, U2);}
