...
// S_L = {(x, y) | (x, y) in S and y <= x}
// S_U = {(x, y) | (x, y) in S and y > x}
// 
...
#define S_L {assert(G1 && y <= x);}
#define S_U {assert(G2 && y > x);}

#define T1_LL T1
#define T2_LL {trans(G2 && y < x, U2);}
#define T1_UU {trans(G1 && y > x + 1, U1);}
#define T2_UU T2
#define T1_UL {trans(G1 && y == x + 1, U1);}
#define T2_LU {trans(G2 && y == x, U2);}
