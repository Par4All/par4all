...
CMODIF
      ...
      READ ( 5,610) ID(1),TEMPI,ID(2),PARDON,ID(3),A0,ID(4),B
      WRITE(12,620) ID(1),TEMPI,ID(2),PARDON,ID(3),A0,ID(4),B
      ...
      READ ( 5,610) ID(1),AIRMU1,ID(2),AIRMU2,ID(3),AIRLA1,ID(4),AIRLA
      WRITE(12,620) ID(1),AIRMU1,ID(2),AIRMU2,ID(3),AIRLA1,ID(4),AIRLA
...
C        CALL EXITA(5)
      ...
      THREAD=THSECT
      IF(CYL.EQ.0.) JSECTR=
      IF(CYL.EQ.1.0 .AND. NY.EQ.1) JSECTR=
...
C      TKEI=CVMGZ(TKEI,TKEI*2.0*(STROKE*RPS)**2,RPM)
      ...
      A0MOM=A
      ANC4O2=ANC4*0.
      IF(T.EQ.0.0 .OR. PGSSW.EQ.0.0) PGS=1.
      RPGS2=1./PGS**
...
C      DTMXCA=CVMGZ(9.9E+9,DTMXCA,RPM)
      ...
      IF(T1IGN.LT.0.0 .AND. RPM.GT.0.0) THEN
        T1IGN=ABS(ATDC-CA1IGN)*RRPS
        TDIGN=ABS(CADIGN)*RRPS
      ENDIF
      T2IGN=T1IGN+TDIGN
      IF(T.EQ.0.0) THEN
        DT=DTI
...
C        CALL EXITA(5)
     ...
     &              ID(4),DCONE(I)
      ...
      DTHNOZ(I)=DTHNOZ(I)*PIO
      TILTXY(I)=TILTXY(I)*PIO
      TILTXZ(I)=TILTXZ(I)*PIO
      CONE(I)  =CONE(I)  *PIO
      DCONE(I) =DCONE(I) *PIO
    ...
    5 CONTINUE
      ...
      IF(T1INJ.LT.0.0 .AND. RPM.GT.0.0) THEN
        T1INJ=ABS(ATDC-CA1INJ) *RRPS
        TDINJ=ABS(CADINJ)*RRPS
      ENDIF
      T2INJ=T1INJ+TDINJ
...
C        CALL EXITA(5)
      ENDIF
      IF(MSHEXT.EQ.1) GO TO
      IF(NHO.EQ.0.AND.MSHEXT.EQ.0) GO TO
CMODIF2
      ZMAX=-1.0E+
      DO 60 N=1,NHO
...
C        CALL EXITA(5)
      ENDIF
      IF(NVZONE.EQ.0) GO TO
      DO 100 N=1,NVZONE
      IF1=(N-1)*6+
      IF6=IF1+
      READ ( 5,660) XEZ(N),YEZ(N),ZEZ(N),(IFACE(I),I=IF1,IF6),IEDG(N)
      WRITE(12,670) XEZ(N),YEZ(N),ZEZ(N),(IFACE(I),I=IF1,IF6),IEDG(N)
      IF(XEZ(N).EQ.XCPLOT) XEZ(N)=XEZ(N)+1.E-
      IF(YEZ(N).EQ.YCPLOT) YEZ(N)=YEZ(N)+1.E-
      IF(ZEZ(N).EQ.ZCPLOT) ZEZ(N)=ZEZ(N)+1.E-
      XDIST=XEZ(N)-XCPLOT
      YDIST=YEZ(N)-YCPLOT
      ZDIST=ZEZ(N)-ZCPLOT
...
C +++
      ...
      Q1=ASIN(YOG)+PIO
      IF(YDIST.GT.0. .AND. XDIST.GT.0.) THETA=Q
      IF(YDIST.GT.0. .AND. XDIST.LT.0.) THETA=-Q
      ZOR=ZDIST/RADEYE
...
C        CALL EXITA(5)
      ENDIF
      IF(NVVVEC.EQ.0 .AND. NY.GT.1) GO TO
C      NVVV=ICVMGT(1,NVVVEC,NVVVEC.EQ.0 .AND. NY.EQ.1)
      DO 140 N=1,NVVV
      IF(NVVVEC.EQ.0 .AND. NY.EQ.1) GO TO
  ...
  120 XEV=0.
      ...
      JSLV(N)=
      KSLV(N)=
  130 IF(XEV.EQ.XCPLOT) XEV=XEV+1.E-
      IF(YEV.EQ.YCPLOT) YEV=YEV+1.E-
      IF(ZEV.EQ.ZCPLOT) ZEV=ZEV+1.E-
      XDIST=XEV-XCPLOT
      YDIST=YEV-YCPLOT
      ZDIST=ZEV-ZCPLOT
      GROUND=AMAX1(SQRT(XDIST**2+YDIST**2),1.E-6)
...
C        CALL EXITA(5)
      ENDIF
      IF(NVCONT.EQ.0 .AND. NY.GT.1) GO TO
C      NVCO=ICVMGT(1,NVCONT,NVCONT.EQ.0 .AND. NY.EQ.1)
      DO 180 N=1,NVCO
      IF(NVCONT.EQ.0 .AND. NY.EQ.1) GO TO
  ...
  160 XEC=0.
      ...
      JSLC(N)=
      KSLC(N)=
  170 IF(XEC.EQ.XCPLOT) XEC=XEC+1.E-
      IF(YEC.EQ.YCPLOT) YEC=YEC+1.E-
      IF(ZEC.EQ.ZCPLOT) ZEC=ZEC+1.E-
      XDIST=XEC-XCPLOT
      YDIST=YEC-YCPLOT
      ZDIST=ZEC-ZCPLOT
      GROUND=AMAX1(SQRT(XDIST**2+YDIST**2),1.E-6)
...
C +++
      IF(TPI.GT.TCRIT .OR. BREAKUP.EQ.0.0 .OR. NUMNOZ.EQ.0) GO TO
...
C +++
      ...
      QR(IR)=0.
      DO 230 ISP=1,NSP
      IF(AM(ISP,IR).EQ.0 .AND. BM(ISP,IR).EQ.0) GO TO
      NK=NK+
      CM(NK,IR)=ISP
...
C        CALL EXITA(5)
      ...
      QEQ(IRE)=0.
      DO 260 ISP=1,NSP
      IF(AN(ISP,IRE).EQ.0 .AND. BN(ISP,IRE).EQ.0) GO TO
      NK=NK+
      CN(NK,IRE)=ISP
...
C +++
      ...
      DO 340 N=1,
      XX=0.12*FLOAT(N)
      RDINJ(N)=(1.0-EXP(-XX)*(1.+XX+.5*XX**2+SIXTH*XX**3))*RRD
  340 CONTINUE
C
C
