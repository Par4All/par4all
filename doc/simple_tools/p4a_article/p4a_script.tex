...
\begin{verbatim}
Executing the CUDA kernel on a GPU with KERNEL_SIZE=8192:
 P4A: Creating block descriptor "P4A_block_descriptor" of size 512x256x1
 P4A: Creating grid of block descriptor "P4A_grid_descriptor" of size 16x32x1
 P4A: Invoking kernel (p4a_kernel_wrapper_0, P4A_block_descriptor, P4A_grid_descriptor) with (i, j, array)
\end{verbatim}
}
...
cope more easily with \Acuda \Agpu. The outer-loop is mapped on the $X$
\Agpu dimension, so it is the dimension that must have enough parallelism.
