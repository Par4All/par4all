...
\begin{frame}{\texttt{p4a} capabilities}
  ...
  \item Scilab parallelization with Wild Cruncher
  \item Code generation for heterogeneous accelerators
    \begin{itemize}
    \item CUDA for GPU
    \item OpenCL for GPU \& ST Platform 2012 (on-going)
    \item SCMP task programs for SCMP machine from CEA
    \end{itemize}
...
\end{alltt}
  ...
  \item \texttt{-{}-com-optimization} use static interprocedural data-flow
    analysis to keep track of data movement and allocation to remove
    redundant communications and allocation
  \item \texttt{-{}-scmp} generates an application for the SCMP architecture
  \end{itemize}
\end{frame}
...
\end{lstlisting}
  ...
  \item No code generation for shared memory. But Fermi SM caches help a
    lot!
  \end{itemize}
\end{trans}
...
\begin{trans}{Do and don't}
  Look at the Par4All coding rules guide
  \begin{itemize}
