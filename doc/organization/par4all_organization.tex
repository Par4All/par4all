...
\item[\texttt{-g} or \texttt{--recursive-git-svn}] apply a \Agit command
  to all the git working copies inside the current directory. If no argument
  is given, a \texttt{git svn rebase} is done;
...
\item[\texttt{-p} or \texttt{--pull-remote-git}] pull the objects from the
  remote \Agit repositories into their respective branches;
\item[\texttt{-m} or \texttt{--merge-remote-git}] merge the branches that
  buffer the remote git repositories into the current branch.
\end{description}
...
\verb|p4a_git --pull-remote-git| pull the \Apolylib into the
...
defined branches:
\begin{description}
\item[\texttt{\emph{numerical}}] a branch corresponding to a given version
  snapshot;
\item[\texttt{\emph{numerical}-alpha}] a branch corresponding to an alpha
  version of a given version snapshot;
\item[\texttt{\emph{numerical}-beta}] a branch corresponding to an beta
  version of a given version snapshot;
\item[{full}] is the branch to get the full latest \Apfa version with all
  the different packages here;
...
\item[\texttt{linear}] points to the import of \Apips \texttt{linear}
...
\item[\texttt{nlpmake}] points to the import of \Apips \texttt{nlpmake}
  part. Thus this branch should contain only the files from
  \texttt{package/PIPS/nlpmake};
\item[\texttt{own}] points to the last development of the \Apfa own files,
  without packages, etc;
\item[\texttt{pips}] points to the import of \Apips \texttt{pips}
...
\item[\texttt{polylib}] points to the import of \Apips \texttt{polylib}
  par. Thus this branch should contain only the files from
