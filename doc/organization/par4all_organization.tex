...
\section{Installation from binary distribution}
\label{sec:inst-from-binary}

\marginpar{TODO : to be generated from RELEASE-NOTES.txt}
...
``.deb`` package you have found on http://download.par4all.org with::
  ...
  sudo gdebi <thepackage>.deb

(sudo dpkg -i <thepackage>.deb would also work but does not automatically
install dependencies.)

An even less automatic way is to use a tar ball ``.tar.gz`` file. It contains
the binaries as built on a stable Ubuntu or Debian distribution. It should
work on any GNU/Linux distribution with the following libraries installed:

  (a fairly recent) libc.so.6, libncurses.so.5, libreadline.so.6
...
You will then need to source one of the following shell scripts which set up
the environment variables for proper Par4All execution:

  source /usr/local/par4all/etc/par4all-rc.sh (if you use bash, sh, dash, etc.)
  source /usr/local/par4all/etc/par4all-rc.csh (if you use csh, tcsh etc.)
...
There are more information in the \verb|RELEASE-NOTES.txt| file at the
root of the \Apfa files.
...
The classical \Aautotools environment variables can be used to
influence\marginpar{More proof-reading needed here!}
...
To speed up the compilation, you can pass options to make, and more
...
\verb|p4a_setup| that has the following options.
...
\Acri:\marginpar{Right now there are not in production yet.}
\begin{itemize}
...
\verb|src/simple_tools/p4a_setup --only=pips -vvv -j 8 --nofinal|
...
\item \verb|optparse_help_to_tex| is a small compiler that transforms a
  ...
  articles or slides. See \S~\ref{sec:docum-help-tex} for more details;
...
Here is the full option list of this command.
...
are processed in the order of class construction, filtering, display and
acceptance, so that you can pipe actions. To be useful,
\verb|pips_validate| should be run with the \texttt{-k} (history keeping)
option before, so that you can operate on differences between different
validations.
...
\verb|p4a_post_processor.py| is the script that transform the \Apips
...
\subsection{The \protect\texttt{optparse\_help\_to\_tex} documentation
  help to \TeX{} compiler}
\label{sec:docum-help-tex}

\verb|optparse_help_to_tex| is a small compiler that translated a help
