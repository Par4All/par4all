...
\url{ssh://git.silkan.com/git/par4all.git}

This repository can be viewed with a \Awww browser at
\url{https://git.silkan.com/cgit/par4all}
...
\Acri:
\begin{itemize}
\item \url{ssh://git.silkan.com/git/svn-linear.git}
\item \url{ssh://git.silkan.com/git/svn-newgen.git}
\item \url{ssh://git.silkan.com/git/svn-nlpmake.git}
\item \url{ssh://git.silkan.com/git/svn-pips.git}
\item \url{ssh://git.silkan.com/git/svn-validation.git}
\end{itemize}
...
\url{ssh://git.silkan.com/git/par4all-private.git}

For \Ahpcp-confidential information,
\url{ssh://git.silkan.com/git/par4all-private-hpc.git} is used.
...
\item[\texttt{doc}] contains the sources of the \Apfa documentation,
  ...
  \begin{description}
  \item[\texttt{developer\_guide}] stores indeed the sources of this document;
  \item[\texttt{installation\_guide}] explains how to install \Apfa;
  ...
  \item[\texttt{simple\_tools/p4a\_slides}] is a very simplified version
    of the previous manual as slides;
  \item[\texttt{user\_guide}] is the user manual of \Apfa;
  \end{description}
...
\item[\texttt{\emph{PREFIX-DIR}}] contains the usable \Apfa is installed
  ...
  \item[\texttt{makes}] contains \texttt{make}-file tools for the
    \Apips validation;
  \item[\texttt{RELEASE-NOTES.rst}] is the release notes of this \Apfa
    instance;
...
\section{Repositories and integration work-flow}
...
workflow and the integration of the different parts.
...
\subsection{The p4a\_git script to deal with workflow and integration}
...
\end{verbatim}
  Note that this instruction and the following work into
  \verb|$P4A_ROOT|, %$
...
\end{verbatim}
  ...
  If you want to skip some commits in one package, say Linear for \Apips,
  it is easier to do a \verb|p4a_git --pull-remote-git| and the go back on
  \texttt{} branch with a \texttt{git branch -f p4a-linear ...};
...
\end{verbatim}
Note that this may be quite verbose, so you may be more comfortable with:
\begin{verbatim}
...
to launch first
\texttt{gdb python} or with Emacs \texttt{}and then run \texttt{p4a} with:
\begin{verbatim}
