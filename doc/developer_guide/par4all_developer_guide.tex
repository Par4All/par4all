...
To start from another branch different from the default
\verb|p4a|-prefixed branch infrastructure, for example
\texttt{p4a-0.2-alpha}, use:
\begin{verbatim}
...
p4a_git --branch-action "git checkout p4a\$suffix; git pull origin p4a\$suffix"
\end{verbatim}
or even faster by avoiding a fetch for each \texttt{pull}:
\begin{verbatim}
p4a_git --branch-action "git checkout p4a\$suffix; git pull origin p4a\$suffix"
\end{verbatim}
...
the end, to test the new \texttt{p4a_git} script for example for the
