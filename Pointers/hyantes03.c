/* Full version of hyantes.c, but the main has been restructured.
 *
 * The main is changed wrt hyantes02: t is not initialized in the
 * declaration, but in a separate statement.
 */
...
void display(towns t,data_t xmin, data_t ymin, data_t xmax, data_t ymax, data_t step)
{
  ...
  for(i=0;i<rangex;i++)
    {
      for(j=0;j<rangey;j++)
	printf(OUTPUT_FORMAT,t.data[i*rangey+j][0],t.data[i*rangey+j][1],t.data[i*rangey+j][2]);
      printf("\n");
    }
}
...
int main(int argc, char * argv[])
{
  ...
  if(argc != 8)
    ...
    /* A memory leak is detected here because the analyzer does not
       know that the block exit is about equivalent to a main function
       exit. */
  }
