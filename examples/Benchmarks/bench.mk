...
CUDA_SOURCE := $(TARGET).naive.cu
CUDA_OPT_SOURCE := $(TARGET).opt.cu
...
$(CUDA_TARGET): $(CUDA_SOURCE) $(COMMON)
	nvcc $^ -o $@ $(COMMON_FLAGS) $(LDFLAGS) $(ACCEL_FLAGS) $(ACCEL_SRC)
cuda: $(CUDA_TARGET)
...
run_seq: $(SEQ_TARGET)
	for run in $(NRUNS); do \
		$(TOP)/scripts/record_measure.sh $(TARGET) $@ `./$< | tee -a $(TARGET)_$@.time`; \
	done
...
# Clean targets
clean: 
	rm -f $(OMP_SOURCE) $(CUDA_SOURCE) $(CUDA_OPT_SOURCE) *.database *.build .*.tmp
dist-clean: clean 
